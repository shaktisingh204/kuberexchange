...
export class MatchDetailComponent implements OnInit, OnDestroy {
      ...
      function (data: any) {
        ...
        if (data) {
          ...
          for (var i = 0; i < this.single_market.length; i++) {
            ...
            //  tv
            if (this.single_market[i].url && this.single_market[i].url !== '') {
              try {
                const url = new URL(this.single_market[i].url);
                ...
                console.warn('Invalid URL format:', this.single_market[i].url);
              }
            }

            // scoreboard
            if (this.single_market[i].score && this.single_market[i].score !== '') {
              try {
                const scoreUrl = new URL(this.single_market[i].score);
                if (scoreUrl.protocol === 'http:' || scoreUrl.protocol === 'https:') {
                  this.graphicTvUrl = this.sanitizer.bypassSecurityTrustResourceUrl(
                    this.single_market[i].score
                  );
                }
              } catch (e) {
                console.warn('Invalid score URL format:', this.single_market[i].score);
              }
            }
          }
    ...
    if (!this.disablePlaceBet) {
      if (this.betPrice == betValue.odds) {
        ...
        this.odds_bookmaker_db.bet["browserdetail"] =
          this.browser_details?.browserdetail || 'Unknown';
        this.odds_bookmaker_db.bet["ipaddress"] =
          this.browser_details?.ipaddress || 'Unknown';
  ...
  saveFancyBet(betValue) {
    ...
    this.session_db.bet["browserdetail"] = this.browser_details?.browserdetail || 'Unknown';
    this.session_db.bet["ipaddress"] = this.browser_details?.ipaddress || 'Unknown';

...
export class UserDataComponent implements OnInit {
  ...
  user_bet_form: any = { userId: "", role: "", status: "" };
  credit_limit_form: any = { userId: "", role: "", credit: "", oldcredit: "" };
  exposure_limit_form: any = {
    userId: "",
    role: "",
    exposure: "",
    oldexposure: "",
  };
  sports_settings_form: any = {
    cricket: "",
    tennis: "",
    soccer: "",
    casino: "",
  ...
  userListBackup: any = [];
  us_bs_bt: boolean;
  cricket_option: boolean;
  soccer_option: boolean;
  tennis_option: boolean;
  casino_option: boolean;
  live_casino_option: boolean;
  down_icon_visible: boolean = true;
  both_active_list: any = [];
  one_ac_inactive_list: any = [];
  both_inactive_list: any = [];
  math = Math;
  ...
  async ngOnInit() {
    ...
    this.applyValidationToFormGroup(
      this.depositWithdrawlForm,
  ...
  getUsrList() {
    ...
    this.sport.Post("getUserList", data).subscribe((res) => {
      // both_active
      const both_active_list = res.response.filter((item) => {
        return item.status == "active" && item.betStatus == true;
      });

      // one active_inactive
      const one_ac_inactive_list = res.response.filter((item) => {
        return (
          (item.status == "inactive" && item.betStatus == true) ||
          (item.status == "active" && item.betStatus == false)
        );
      });
      // both_inactive
      const both_inactive_list = res.response.filter((item) => {
        return item.status == "inactive" && item.betStatus == false;
      });

      this.usrList = [].concat(
        both_active_list,
        one_ac_inactive_list,
        both_inactive_list
      );
      this.total_items = res.totalUsers;
  ...
  getTotalBalReport() {
    ...
    this.sport.Post("getBalanceReport", data).subscribe((res) => {
      if (res.success) {
        ...
        console.log('getTotalBalReport',this.get_list_top_bal)
      } else {
        this.toastr.error(res.msg, "", {
          timeOut: 10000,
        });
  ...
  userNameClickList(username: string, role: string, usrId: string) {
    if (!(role === "user")) {
      ...
      sessionStorage.setItem(
        "userlistBackData",
        JSON.stringify(this.userListBackup)
      );

      const data = { role: role, userId: usrId, pageNumber: 1, limit: 50 };
      this.sport.Post("getUserList", data).subscribe((res) => {
        if (res.success) {
          // this.usrList=[];
          // both_active
          ...
          if (
            res.response[0].ParentUser === this.loggedInUser.details.username
          ) {
            this.down_icon_visible = true;
          } else {
            this.down_icon_visible = false;
          }
          this.get_userList_totalBal(username, role);
        } else {
          this.toastr.error(res.msg, "", {
            timeOut: 10000,
          });
  ...
  get_userList_totalBal(username: string, role: string) {
    // changeskrne hai
    ...
    this.sport.Post("getBalanceReport", data).subscribe((res) => {
      if (res.success) {
        ...
        this.get_list_top_bal = res.response;
        console.log('get_userList_totalBal',this.get_list_top_bal)
      } else {
        this.toastr.error(res.msg, "", {
          timeOut: 10000,
        });
  ...
  getManagerBal(parent_usrId: string) {
    const data = {
      ...
      targetUser: { userId: parent_usrId },
    };

    this.Socket.emit("get-parentuser-balance", data);
    this.Socket.on(
      "parentuser-balance-success",
      function (data: any) {
        this.manager_bal = 0;
  ...
  Withdraw(sharing: TemplateRef<any>) {
    ...
    if (amount < 0) {
      ...
      const data = {
        transpassword: this.transcation_password,
        targetUser: this.withdrawl_form,
      };
  ...
  update_Exposure() {
    const data = {
      transpassword: this.transcation_password,
      targetUser: this.exposure_limit_form,
    };
  ...
  update_Credit() {
    const data = {
      transpassword: this.transcation_password,
      targetUser: this.credit_limit_form,
    };
  ...
  usr_status_toggle(value: any) {
    ...
    if (value) {
      ...
      //  console.warn( this.user_status_form.status);
    } else {
  ...
  usr_bet_toggle(value: any) {
    this.us_bs_bt = false;
    if (value) {
      this.user_bet_form.status = "true";
      //  console.warn(this.user_bet_form.status);
    } else {
      this.user_bet_form.status = "false";
      // console.warn(this.user_bet_form.status);
    }
  }
  ...
  changeUsrStatus() {
    if (this.us_bs_bt) {
      const data = {
        transpassword: this.transcation_password,
        targetUser: this.user_status_form,
      };
  ...
  changeBetStatus() {
    const data = {
      transpassword: this.transcation_password,
      targetUser: this.user_bet_form,
    };

    this.sport.Post("updateBetStatus", data).subscribe((res) => {
      //  console.warn(res);
  ...
  sport_toggle(value: boolean, type: string) {
    if (type === "4") {
      if (value) {
        this.sports_settings_form.cricket = "4";
      } else {
        this.sports_settings_form.cricket = "";
      }
    } else if (type === "2") {
      if (value) {
        this.sports_settings_form.tennis = "2";
      } else {
        this.sports_settings_form.tennis = "";
      }
    } else if (type === "1") {
      if (value) {
        this.sports_settings_form.soccer = "1";
      } else {
        this.sports_settings_form.soccer = "";
      }
    } else if (type === "c9") {
      if (value) {
        this.sports_settings_form.casino = "c9";
      } else {
        this.sports_settings_form.casino = "";
      }
    } else if (type === "c1") {
      if (value) {
        this.sports_settings_form.live_casino = "c1";
      } else {
        this.sports_settings_form.live_casino = "";
      }
    }
  }
  ...
  Sports_settings() {
    const data = {
      transpassword: this.transcation_password,
      status: this.sports_settings_form,
      targetUser: this.user_bet_form,
    };
    this.sport.Post("updateSportEvents", data).subscribe((res) => {
      console.warn(res);
      if (res.success) {
        this.toastr.success(res.message, "Success!");
        this.modalRef.hide();
  ...
  downlineList(user) {
    ...
    this.router.navigate(
      ["downline-report/" + user._id + "/" + user.user_type_id],
      {
        queryParams: {
          netExposure: JSON.stringify(user.liability),
          balance: JSON.stringify(user.balance),
          profit_loss: JSON.stringify(user.profit_loss),
        },
      }
    );
  }
  ...
  userMobilePermissionSetting(index) {
    ...
    this.mobileDisplayPermissionBox[index] =
      !this.mobileDisplayPermissionBox[index];
    setTimeout(() => {
      this.closeUserPermissionDiv(index);
    }, 15000);
  ...
  openModalPermissionSetting(
    permissionSetting: TemplateRef<any>,
    usrname: any,
    status: any,
    role: any,
    b_status: any,
    ...
    this.bet_status = b_status;
    this.user_status_form.userId = usrId;
    this.user_status_form.status = status;
    this.user_status_form.role = role;
    this.user_bet_form.userId = usrId;
    this.user_bet_form.status = status;
    this.user_bet_form.role = role;
  ...
  // more_button
  openModalMoreSetting(
    sportsSetting: TemplateRef<any>,
    usrId: any,
    status: any,
    role: any,
    parentUsr: string
    ...
    const data = { targetUser: this.user_bet_form };
    this.sport.Post("getUserEvenets", data).subscribe((res) => {
      if (res.success) {
        // show_option
        if (res.parent.length == 0) {
          this.cricket_option = true;
          this.soccer_option = true;
          this.tennis_option = true;
          this.casino_option = true;
          this.live_casino_option = true;
        } else {
          this.cricket_option = res.parent.includes("4");
          this.tennis_option = res.parent.includes("2");
          this.soccer_option = res.parent.includes("1");
          this.casino_option = res.parent.includes("c9");
          this.live_casino_option = res.parent.includes("c1");
        }

        // show_status
        const cricket = res.response.availableEventTypes.includes("4");
        const tennis = res.response.availableEventTypes.includes("2");
        const soccer = res.response.availableEventTypes.includes("1");
        const casino = res.response.availableEventTypes.includes("c9");
        const live_casino = res.response.availableEventTypes.includes("c1");
  ...
  async openModalRawPwd(rawPwd: TemplateRef<any>, userid, index) {
    ...
    if (!this.cookie.get("transaction-password")) {
      ...
      this.transactionPassword = await this.usersService.decryptData(
        this.cookie.get("transaction-password")
      );
      this.getUserRawPassword();
    }
  }

  openModalUserPwd(userPwd: TemplateRef<any>, usrId: any, role: any) {
    this.change_password_form.userId = usrId;
  ...
  updatePasswd() {
    if (
      ...
      const data = {
        token: this.adminDetails.apitoken,
        transpassword: this.transcation_password,
        targetUser: this.change_password_form,
      };
      this.Socket.emit("update-userpassword", data);
      this.Socket.on(
        "update-userpassword-success",
        function (data: any) {
          if (data.success) {
            ...
            this.transcation_password = "";
            this.modalRef.hide();
            this.socket.removeAllListeners("update-userpassword-success");
          } else {
  ...
  getUserAndParentDetails(id) {
    this.usersService.getUserDetailsWithParentDetails(id).subscribe(
      (data) => {
        if (data.status == true) {
  ...
  depositWithdrawl(id, parentId, crdr) {
    if (this.depositWithdrawlForm.controls.accChips.value) {
      if (this.depositWithdrawlForm.controls.userPass.value) {
        ...
        if (this.isSocket != 1) {
          this.usersService.depositWithdrawl(this.userChipsData).subscribe(
            (data) => {
              if (data.status == true) {
  ...
  getUserChildDetail(id, disableHierarchyadding?: boolean) {
    ...
    if (this.isSocket != 1) {
      this.usersService
        .getUserChildDetail(id, this.usersListReqPageQuery)
        .subscribe(
          (data) => {
            if (data.status == true) {
              this.addUserId = id;
              ...
              this.expoTotal = this.userList.reduce(
                (a: number, b) => a + b.liability,
                0
              );
              this.plTotal = this.userList.reduce(
                (a: number, b) => a + b.profit_loss,
                0
              );
              this.balTotal = this.userList.reduce(
                (a: number, b) => a + b.balance,
                0
              );
              ...
              if (this.loggedInUser.user_type_id == 0) {
                if (this.childLevelFilterValues.length == 0) {
                  for (
                    let i = 0;
                    i < this.adminDetails.highestNumberChild;
                    i++
                  ) {
              ...
              if (disableHierarchyadding) {
                ...
                if (
                  this.previousRouteService
                    .getPreviousUrl()
                    .includes("/addChild-agent/") ||
                  this.previousRouteService
                    .getPreviousUrl()
                    .includes("/addChild-user/") ||
                  this.previousRouteService
                    .getPreviousUrl()
                    .includes("/user-block-market/") ||
                  this.previousRouteService
                    .getPreviousUrl()
                    .includes("/downline-report")
                ) {
                  let savedHierarchy = JSON.parse(
                    sessionStorage.getItem("hierarchy")
                  );
                  this.hierarchy = savedHierarchy;
                  this.onUserNameClicked(
                    savedHierarchy[savedHierarchy.length - 1].userId,
                    savedHierarchy[savedHierarchy.length - 1].user_type_id,
                    "onclickusername",
                    savedHierarchy[savedHierarchy.length - 1].page
                  );
  ...
  downloadCsv() {
    ...
    for (var i = 0; i < this.usrList?.length; i++) {
      let x = {
        'UserName': this.usrList[i].username,
        'Credit Referance': this.usrList[i].creditrefrence,
        'Balance': this.usrList[i].totalbalance,
        'Client (P/L)': (this.usrList[i].totalbalance - this.usrList[i].creditrefrence),
        'Exposure': (this.usrList[i].totalexposure * -1),
        'Available Balance': this.usrList[i].balance,
        'U St': this.usrList[i].status,
        'B St': this.usrList[i].betStatus,
        'Exposure Limit': this.usrList[i].exposurelimit,
        'Defalut %': this.usrList[i].partnershipsetting[0]?.partnership,
        'Account Type': this.usrList[i].role,
        'Casino Total': this.usrList[i].casinobalance,
      }
      arr.push(x);
  ...
  getSubUserChild(id, type, fromIndex?: any) {
    if (this.isSocket != 1) {
      ...
      this.usersService
        .getUserChildDetail(this.levelParentUserId, this.usersListReqPageQuery)
        .subscribe(
          (data) => {
  ...
  changeChildPwd(userId, parentId) {
    ...
    if (this.isSocket != 1) {
      ...
      this.usersService.changeChildPassword(this.data).subscribe(
        (data) => {
          if (data.status == true) {
            this.userParentData = data.data;
            this.getUserChildDetail(parentId, false);
  ...
  onSubmitChangePassword() {
    ...
    if (
      ...
      this.usersService
        .changePassword(this.selectedUserId, this.changePasswordForm.value)
        .subscribe(
          (result) => {
            if (result.status == true) {
              ...
              if (
                this.levelParentUserId != null &&
                this.levelParentUserId != undefined
              ) {
                this.getSubUserChild(this.levelParentUserId, null);
              } else {
                this.getUserChildDetail(this.user_id, false);
  ...
  lockAccountOfUserAndTheirChilds(userid, self_lock_user) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (this.isSocket != 1) {
          this.usersService.lockUserAccount(this.selectedUserId, obj).subscribe(
            (result) => {
              if (result.status == true) {
                if (
                  this.levelParentUserId != null &&
  ...
  closeAccountOfUserAndTheirChilds(
    userid,
    self_close_account,
    balance,
    liability,
    ...
    if (balance == 0 && liability == 0 && pl == 0) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          if (this.isSocket != 1) {
            this.usersService
              .closeAndReOpenAccountOfUserAndTheirChilds(
                this.selectedUserId,
                obj
              )
              .subscribe(
      ...
      alert(
        "First Clear Exposure , Balance & Settlement amount. After then you will close A/C successfully...."
      );
      this.getUserChildDetail(this.user_id, false);
    }
  }
  ...
  allowAndNotAllowAgentsMultiLogin(user_id) {
    ...
    var obj: any = {};
    obj.user_id = user_id;
    this.usersService.allowAndNotAllowAgentsMultiLogin(obj).subscribe(
      (result) => {
        if (result.status == true) {
          if (
            this.levelParentUserId != null &&
            ...
            this.getSubUserChild(this.levelParentUserId, null);
          else this.getUserChildDetail(this.user_id, false);
          this.toastr.success(result.msg, "", {
            positionClass: "toast-bottom-right",
            timeOut: 1000,
          });
  ...
  applyValidationToFormGroup(formGroupName, jsonArrayName) {
    this.appValidationService
      .applyValidationRulesToFromGroup(formGroupName, jsonArrayName)
      .then((validators) => { })
      .catch(() => { });
  }
  ...
  getPartnershipListByUserId(userId, username) {
    ...
    if (this.isSocket != 1) {
      this.usersService.getPartnershipListByUserId(obj).subscribe(
        (result) => {
          if (result.status == true) {
            this.selectedUserPartnershipList = result.data.sports_share;
            for (var i = 0; i < this.selectedUserPartnershipList.length; i++) {
              this.addedSportShareList.push({
                sport: this.selectedUserPartnershipList[i].sport,
                sport_id: this.selectedUserPartnershipList[i].sport_id,
                name: this.selectedUserPartnershipList[i].name,
                percentage: [],
              });
              for (
                var j = 0;
                j <= this.selectedUserPartnershipList[i].percentage.length;
                j++
              ) {
                if (j < this.selectedUserPartnershipList[i].percentage.length) {
                  var percentage =
                    this.selectedUserPartnershipList[i].percentage[j];
  ...
  checkUserSportShareForDisplay() {
    ...
    for (var i = 0; i < this.selectedUserPartnershipList.length; i++) {
      this.tempUserShareSportList.push({
        userCurrentShare:
          this.selectedUserPartnershipList[i].percentage[
            this.selectedUserPartnershipList[i].percentage.length - 1
          ].user_share,
      });
    }
  }
  ...
  updatePartnershipList() {
    if (this.isUpdateShareValid) {
      if (
        this.updatePartnershipPassword == null ||
        this.updatePartnershipPassword == undefined ||
        this.updatePartnershipPassword == ""
      ) {
        this.toastr.error("Please enter password", "", {
          timeOut: 10000,
        });
      ...
      if (this.isSocket != 1) {
        this.usersService.updatePartnershipList(updatePartnershipObj).subscribe(
          (result) => {
            if (result.status == true) {
              this.toastr.success(result.msg, "", {
                positionClass: "toast-bottom-right",
  ...
  checkUserEnteredShare(user_share, parent_share, sport, index) {
    if (user_share > parent_share) {
      ...
      this.addedSportShareList[index].percentage[
        this.addedSportShareList[index].percentage.length - 1
      ].user_share = parseInt(user_share);
      this.addedSportShareList[index].percentage[
        this.addedSportShareList[index].percentage.length - 1
      ].share = parseInt(user_share);
      this.addedSportShareList[index].percentage[
        this.addedSportShareList[index].percentage.length - 1
      ].parent_partnership_share =
        parseInt(
          this.addedSportShareList[index].percentage[
            this.addedSportShareList[index].percentage.length - 2
          ].parent_share
        ) - parseInt(user_share);
      this.addedSportShareList[index].percentage[
        this.addedSportShareList[index].percentage.length - 2
      ...
      this.addedSportShareList[index].percentage[
        this.addedSportShareList[index].percentage.length - 1
      ].parent_share = parseInt(user_share);
      this.addedSportShareList[index].percentage[
        this.addedSportShareList[index].percentage.length - 2
      ].user_share = parseInt(user_share);
    } else {
  ...
  checkedBetAllow(event, user_id, parentId) {
    if (this.isSocket != 1) {
      this.usersService
        .updateUserBettingLockUnlock(user_id)
        .subscribe((result) => {
          if (result.status == true) {
            this.toastr.success(result.msg, "", {
              positionClass: "toast-bottom-right",
  ...
  checkedFancyBetAllow(event, user_id, parentId, userSelf_lock_fancy_bet) {
    ...
    if (userSelf_lock_fancy_bet == 0) {
      if (this.isSocket != 1) {
        this.usersService
          .updateUserStatusFancyBetLock(data)
          .subscribe((result) => {
            if (result.status == true) {
              this.toastr.success(result.msg, "", {
                positionClass: "toast-bottom-right",
      ...
      if (this.isSocket != 1) {
        this.usersService
          .updateUserStatusFancyBetUnlock(data)
          .subscribe((result) => {
            if (result.status == true) {
              this.toastr.success(result.msg);
              this.getUserChildDetail(parentId, false);
  ...
  createSportsSettingArray(user?: any) {
    ...
    this.specificUserDetails.forEach((sport, index) => {
      this.sportsSettingsFormArr.push(
        this.fb.group({
          ...
          market_min_odds_rate: [
            sport.market_min_odds_rate,
            Validators.required,
          ],
          market_max_odds_rate: [
            sport.market_max_odds_rate,
            Validators.required,
          ],
          market_advance_bet_stake: [
            sport.market_advance_bet_stake,
            Validators.required,
          ],
      ...
      if (!user.check_event_limit) {
        this.sportsSettingsFormArr.controls[index][
          "controls"
        ].market_min_stack.setValidators([
          ValidatorControls.requiredValidator("Market Min Stack is required"),
          ValidatorControls.minValueValidator(
            1,
            true,
            "Market Min Stack value should not be less than or equal to 0",
            false
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
        ].market_max_stack.setValidators([
          ValidatorControls.requiredValidator("Market Max Stack is required"),
          ValidatorControls.minValueValidator(
            this.sportsSettingsFormArr.controls[index]["controls"]
              .market_min_stack.value,
            true,
            "Market Max Stack value should not be less than Market Min Stack",
            false
          ),
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].market_max_stack_max_limit,
            true,
            "Market Max Stack value should not be greater than " +
            this.specificUserParentDetails[index].market_max_stack_max_limit
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
        ].market_min_odds_rate.setValidators([
          ValidatorControls.requiredValidator(
            "Market Min odds rate is required"
          ),
          ValidatorControls.minValueValidator(
            this.specificUserParentDetails[index].market_min_odds_rate,
            true,
            "Market Min odds rate value should not be less " +
            this.specificUserParentDetails[index].market_min_odds_rate,
            false
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
        ].market_max_odds_rate.setValidators([
          ValidatorControls.requiredValidator(
            "Market Max odds rate is required"
          ),
          ValidatorControls.minValueValidator(
            this.sportsSettingsFormArr.controls[index]["controls"]
              .market_min_odds_rate.value,
            true,
            "Market Max odds rate value should not be less than Market Min odds rate",
            false
          ),
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].market_max_odds_rate,
            true,
            "Market Max odds rate value should not be greater than " +
            this.specificUserParentDetails[index].market_max_odds_rate
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
        ].market_max_profit.setValidators([
          ValidatorControls.requiredValidator("Market Max profit is required"),
          ValidatorControls.minValueValidator(
            this.specificUserParentDetails[index].market_profit_range,
            true,
            "Market Max profit value should not be less than " +
            this.specificUserParentDetails[index].market_profit_range,
            false
          ),
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].market_max_profit,
            true,
            "Market Max Profit rate value should not be greater than " +
            this.specificUserParentDetails[index].market_max_profit
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          ...
          ValidatorControls.requiredValidator(
            "Before Inplay Match Stake is required"
          ),
          ValidatorControls.minValueValidator(
            1,
            true,
            "Before Inplay Match Stake value should not be less than or equal to 0 ",
            false
          ),
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].market_advance_bet_stake,
            true,
            "Before Inplay Match Stake value should not be more than " +
            this.specificUserParentDetails[index].market_advance_bet_stake
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
        ].market_bet_delay.setValidators([
          ValidatorControls.requiredValidator("Match Bets Delay is required"),
          ValidatorControls.minValueValidator(
            this.specificUserParentDetails[index].market_min_bet_delay,
            true,
            "Match Bets Delay value should not be less than " +
            this.specificUserParentDetails[index].market_min_bet_delay,
            false
          ),
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].market_max_bet_delay,
            true,
            "Match Bets Delay value should not be more than " +
            this.specificUserParentDetails[index].market_max_bet_delay
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
        ].session_min_stack.setValidators([
          ValidatorControls.requiredValidator("Min. Stake Amount  is required"),
          ValidatorControls.minValueValidator(
            1,
            true,
            "Min. Stake Amount value should not be less than or equal to 0",
            false
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
        ].session_bet_delay.setValidators([
          ValidatorControls.requiredValidator("Session Bet Delay is required"),
          ValidatorControls.minValueValidator(
            this.specificUserParentDetails[index].session_min_bet_delay,
            true,
            "Session Bet Delay value should not be less than or equal to " +
            this.specificUserParentDetails[index].session_min_bet_delay,
            false
          ),
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].session_max_bet_delay,
            true,
            "Session Bet Delay value should not be more than " +
            this.specificUserParentDetails[index].session_max_bet_delay
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
        ].session_max_stack.setValidators([
          ValidatorControls.requiredValidator("Max. Stake Amount is required"),
          ValidatorControls.minValueValidator(
            this.sportsSettingsFormArr.controls[index]["controls"]
              .session_min_stack.value,
            true,
            "Max. Stake Amount value should not be less than min Stake Amount value",
            false
          ),
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].session_max_stack_max_limit,
            true,
            "Max. Stake Amount value should not be greater than " +
            this.specificUserParentDetails[index].session_max_stack_max_limit
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
        ].session_max_profit.setValidators([
          ValidatorControls.requiredValidator("Session Max Profit is required"),
          ValidatorControls.minValueValidator(
            this.specificUserParentDetails[index].session_profit_range,
            true,
            "Session Max Profit value should not be less than " +
            this.specificUserParentDetails[index].session_profit_range,
            false
          ),
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].session_max_profit,
            true,
            "Session Max Profit rate value should not be greater than " +
            this.specificUserParentDetails[index].session_max_profit
          ),
        ]);
      } else {
        this.sportsSettingsFormArr.controls[index][
          ...
          ValidatorControls.requiredValidator("Market Min Stack is required"),
          ValidatorControls.minValueValidator(
            this.specificUserParentDetails[index].market_min_stack,
            true,
            "Market Min Stack value should not be less than parent value",
            false
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          ...
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].market_max_stack,
            true,
            "Market Max Stack value should not be greater than parent value"
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
          ...
          ValidatorControls.minValueValidator(
            this.specificUserParentDetails[index].market_min_odds_rate,
            true,
            "Market Min odds rate value should not be less than parent value",
            false
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          ...
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].market_max_odds_rate,
            true,
            "Market Max odds rate value should not be greater than parent value"
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
          ...
          ValidatorControls.minValueValidator(
            1,
            true,
            "Market Max profit value should not be less than or equal to 0 ",
            false
          ),
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].market_max_profit,
            true,
            "Market Max profit value should not be greater than parent value"
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
          ...
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].market_advance_bet_stake,
            true,
            "Before Inplay Match Stake value should not be greater than parent value"
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
          ...
          ValidatorControls.requiredValidator("Match Bets Delay is required"),
          ValidatorControls.minValueValidator(
            this.specificUserParentDetails[index].market_bet_delay,
            true,
            "Match Bets Delay value should not be less than parent value",
            false
          ),
          ValidatorControls.maxValueValidator(
            10,
            true,
            "Match Bets Delay value should not be more than 10"
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
          ...
          ValidatorControls.requiredValidator("Min. Stake Amount  is required"),
          ValidatorControls.minValueValidator(
            this.specificUserParentDetails[index].session_min_stack,
            true,
            "Min. Stake Amount value should not be less than parent value",
            false
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          ...
          ValidatorControls.requiredValidator("Session Bet Delay is required"),
          ValidatorControls.minValueValidator(
            this.specificUserParentDetails[index].session_bet_delay,
            true,
            "Session Bet Delay value should not be less than parent value",
            false
          ),
          ValidatorControls.maxValueValidator(
            10,
            true,
            "Session Bet Delay value should not be more than 10"
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
          ...
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].session_max_stack,
            true,
            "Max. Stake Amount value should not be greater than parent value"
          ),
        ]);
        this.sportsSettingsFormArr.controls[index][
          "controls"
          ...
          ValidatorControls.minValueValidator(
            1,
            true,
            "Session Max Profit value should not be less than or equal to 0 ",
            false
          ),
          ValidatorControls.maxValueValidator(
            this.specificUserParentDetails[index].session_max_profit,
            true,
            "Session Max Profit value should not be greater than parent value"
          ),
        ]);
        // });
      }
  ...
  updateSportWiseSettingDetails(sport, sportIndex) {
    let compareResult = this.objectsAreSame(
      this.sportsForm.controls.sports_settings.value[sportIndex],
      this.specificUserDetails[sportIndex]
    );
    if (!compareResult.objectsAreSame) {
      ...
      let data = {
        ...
        sport_id:
          this.sportsForm.controls.sports_settings.value[sportIndex].sport_id,
      ...
      if (this.isSocket != 1) {
        this.usersService
          .updateSportWiseSettingDetails(data)
          .subscribe((result) => {
            if (result.status == true) {
  ...
  getUserSportsWiseSettingDetails(user_id, settingId, user) {
    if (this.isSocket != 1) {
      this.usersService
        .getSportSetting({ user_id: user_id, _id: settingId })
        .subscribe((result) => {
          this.specificUserDetails = result.data.sports_settings;
          if (
            result.data.sports_settings.length !==
            result.data.parent_sports_settings.length
          ) {
            this.specificUserParentDetails = [];
            this.specificUserDetails.forEach((element, index) => {
              this.specificUserParentDetails.push(
                result.data.parent_sports_settings[0]
              );
            });
          } else {
  ...
  blockMaster(user) {
    sessionStorage.setItem("hierarchy", JSON.stringify(this.hierarchy));
    this.router.navigate([
      "user-block-market/" + user._id + "/" + user.user_type_id,
    ]);
  }
  ...
  async getUserRawPassword() {
    ...
    this.usersService
      .getUserRawPassword(rawPasswordObj)
  ...
  applyValidatorsForMaxStack(index) {
    ...
    this.sportsSettingsFormArr.controls[index][
      "controls"
    ].market_max_stack.updateValueAndValidity();
  }
  ...
  applyValidatorsForMaxOddsRate(index) {
    if (this.sportSettingParentValidation) {
      this.sportsSettingsFormArr.controls[index][
        ...
        ValidatorControls.requiredValidator("Market Max odds rate is required"),
        ValidatorControls.minValueValidator(
          this.sportsSettingsFormArr.controls[index]["controls"]
            .market_min_odds_rate.value,
          true,
    ...
    this.sportsSettingsFormArr.controls[index][
      "controls"
    ].market_max_odds_rate.updateValueAndValidity();
  }
  ...
  applyValidatorsForMaxstake(index) {
    ...
    this.sportsSettingsFormArr.controls[index][
      "controls"
    ].session_max_stack.updateValueAndValidity();
  }
  ...
  filterList(parameter, sortOrder) {
    if (parameter == "username") {
      ...
      if (sortOrder == "asc") {
        this.usrList = this.usrList.sort((a, b) =>
      ...
      if (sortOrder == "asc") {
        this.usrList = this.usrList.sort((a, b) => a[parameter] - b[parameter]);
      } else if (sortOrder == "desc") {
        this.usrList = this.usrList.sort((a, b) => b[parameter] - a[parameter]);
      }
    }
  }
  ...
  updateUserCommission() {
    if (
      this.UserCommissinSettings.match_commission != this.marketCommission ||
      this.UserCommissinSettings.session_commission != this.sessionCommission
    ) {
      let request = {
  ...
  casino_iframe(id: any) {
    const data = { userId: id };
    this.sport.Post("userCasinoHistory", data).subscribe((res) => {
      if (res.success) {
  ...
  refreshBal(id: any) {
    this.sport.Get("refreshBalance/" + id).subscribe((res) => {
      if (res.success) {

...
export class EventDetailsComponent implements OnInit {
  ...
  getUsrlist() {
    const data = {
      role: this.adminDetails.details.role, userId: this.adminDetails._id, "pageNumber": 1,
      "limit": 10000
    };
  ...
  getRunnerBookProfit() {
    this.userList = [];
    this.Socket.emit('get-bookermaker-book', { user: this.adminDetails.apitoken, eventId: this.eventId });
  ...
  // open_createMarket_modal
  addMarketModal(add_market: TemplateRef<any>) {
    this.modalRef = this.modalService.show(
      add_market,
  ...
  // create_Market
  createMarket() {
    if (this.confirmFun('Are you sure you want to create a new market ?')) {
      if (this.cMrkt_marketName && this.cMrkt_back && this.cMrkt_lay) {
        const data = { marketName: this.cMrkt_marketName, back: this.cMrkt_back, lay: this.cMrkt_lay };
        this.Socket.emit('create-market', { token: this.adminDetails.apitoken, eventId: this.eventId, newMarket: data });
        console.warn('hit');

        this.Socket.on('create-market-success', (function (res: any) {
          if (res) {
            this.toastr.success('Create Market Success');
            this.modalRef.hide();
            this.removeAllListeners('create-market-success');
          }
        ...
        this.toastr.error('All field are reqired!');
      }
    }
  ...
  delBets() {
    ...
    if (this.confirmFun('Are you sure you want to delete ' + this.marketRunner.marketName + ' market ?')) {

...
export class DownlineReportComponent implements OnInit,OnDestroy {
  ...
  liability: any;
  amount: any;
  constructor(private locationBack: Location,private toastr: ToastrService,private sport: SportService, private cookie: CookieService,private loginService: LoginService,
  ...
  ngOnInit() {
    ...
    if(this.userExposure == undefined){
      this.openBets();
...
//exposure tab code 
  ...
  openModalUpdateData(updateData: TemplateRef<any>) {
    this.modalRef = this.modalService.show(
      updateData,
      Object.assign({}, { class: 'modal-lg' })
    );

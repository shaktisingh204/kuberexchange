...
////////// ---------- Used Api Socket ---------- //////////
module.exports.getHomeMarkets = async function (io, socket, req) {

  try {
    // console.log("getHomeMarkets", req.token)
    ...
    logger.info("getHomeMarkets: " + JSON.stringify(req));
    // console.log("getHomeMarkets: " + JSON.stringify(req));
    let filter = {
    ...
    Market.find(filter, { competitionId: 1, competitionName: 1, eventId: 1, eventName: 1, eventTypeId: 1, eventTypeName: 1, marketBook: 1, marketName: 1, marketType: 1, openDate: 1, imgArr: 1, runners: 1 }).sort({ openDate: 1 })
      .then(async result => {
        let AllMarkets = [];
        if (!result) {
      ...
      .catch(error => {
        // console.log(error)
        socket.emit("get-homemarkets-success", error);
      })
  }
  catch (error) {
    // console.log(error);
...
module.exports.getInPLayMarkets = async function (io, socket, req) {

  try {
    // console.log("getInPLayMarkets", req.token)
    // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    let { userId } = jwt.decode(req.token);
    let user = await User.findOne({ _id: userId, token: req.token });
    if (!user) return io.to(socket.id).emit('logout');
    ...
    logger.info("getInPLayMarkets: " + JSON.stringify(req));
    // console.log("getInPLayMarkets: " + JSON.stringify(req));
    let filter = {
      eventTypeId: { $nin: ['t9', '4321'] },
      visible: true,
      deleted: false,
      marketType: { $in: ['MATCH_ODDS', 'TOURNAMENT_WINNER'] },
      'marketBook.status': { $ne: 'CLOSED' },
      $or: [{ 'marketBook.inplay': true }, { openDate: { $lte: new Date() } }]
    };
    ...
    Market.find(filter, { competitionId: 1, competitionName: 1, eventId: 1, eventName: 1, eventTypeId: 1, eventTypeName: 1, marketBook: 1, marketName: 1, marketType: 1, openDate: 1, imgArr: 1, runners: 1 }).sort({ openDate: 1 })
      .then(async result => {
        ...
        if (!result) {
          ...
          let tenis = result.filter(item => item.eventTypeId == "2");
          AllMarkets.push(tenis);
          // let virtual = result.filter(item => item.eventTypeId == "v9");
          // AllMarkets.push(virtual);
      ...
      .catch(error => {
        // console.log(error)
        socket.emit("get-inplaymarkets-success", error);
      })
...
module.exports.getMarketById = async function (io, socket, req) {
  // // console.log(request);
  try {
    // // console.log(req)
    // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    ...
    if (!user) return io.to(socket.id).emit('logout');
    if (user.token != req.token) return io.to(socket.id).emit('logout');
    logger.info("getMarkets: " + JSON.stringify(req));
    ...
    Market.find(filter).then(async result => {
      let AllMarkets = [];
      if (!result) {
        socket.emit("get-marketbyid-success", AllMarkets);
      }
      else {
        socket.emit("get-marketbyid-success", result);
      }
    }).catch(error => {
      // console.log(error)
      socket.emit("get-marketbyid-success", error);
    })
  }
  catch (error) {
    // console.log(error);
...
module.exports.betMarkets = async function (io, socket, req) {
  try {
    // console.log(req)
    // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    let { userId } = jwt.decode(req.token);
    ...
    Bet.distinct("marketId", {
      ...
      Market.find(filter).sort({ 'eventTypeId': -1 }).then(async result => {
        // // console.log(result);
        socket.emit("get-betmarkets-success", result);
      });
    }).catch(error => {
      // console.log(error)
      socket.emit("get-betmarkets-success", error);
    })
  }
  catch (error) {
    // console.log(error);
...
module.exports.getVirtualResult = async function (io, socket, req) {
  try {
    // // console.log(req)
    // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    let { userId } = jwt.decode(req.token);
    ...
    if (!user) return io.to(socket.id).emit('logout');
    if (user.token != req.token) return io.to(socket.id).emit('logout');
    logger.info("getVirtualResult: " + JSON.stringify(req));
...
module.exports.getFreeHomeMarkets = function (io, socket, request) {
  if (!request) return;
  if (!request.filter || !request.sort) return;
  // request.filter.marketBook.inplay = true;
  // // console.log(request.filter)
  var filter = request.filter;
  var ds = new Date();
  var de = new Date();
  ...
  filter = {
    ...
    marketType: { '$in': ['MATCH_ODDS', 'TOURNAMENT_WINNER'] },
    'marketBook.status': { '$ne': 'CLOSED' },
    $or: [{ 'marketBook.inplay': true }, { openDate: { $lt: (de.toISOString()) } }]
    // openDate:{$gt:(ds.toISOString()) , $lt:(de.toISOString()) },
    // openDate:{$gt:(ds.toISOString())}
    // 'marketBook.inplay': true,      
    // $or: [{openDate:{$lt:(de.toISOString())} , openDate:{$gt:(ds.toISOString())}}]
  }
  // // console.log(filter)
  Market.find(filter, { competitionId: 1, competitionName: 1, eventId: 1, eventName: 1, eventTypeId: 1, eventTypeName: 1, marketBook: 1, marketName: 1, marketType: 1, openDate: 1, imgArr: 1, runners: 1 }).sort(request.sort).exec(function (err, result) {
    if (err) logger.debug(err);
    // // console.log(result);
    let AllMarkets = [];
    // let in_Play = result.filter(item => item.marketBook.inplay == true);
...
module.exports.getFreeHomeAllMarkets = function (io, socket, request) {
  if (!request) return;
  if (!request.filter || !request.sort) return;
  // request.filter.marketBook.inplay = true;
  // // console.log(request.filter)
  ...
  Market.find(filter, { competitionId: 1, competitionName: 1, eventId: 1, eventName: 1, eventTypeId: 1, eventTypeName: 1, marketBook: 1, marketName: 1, marketType: 1, openDate: 1, imgArr: 1, runners: 1 }).sort(request.sort).exec(function (err, result) {
    ...
    socket.emit("get-freehomeallmarkets-success", AllMarkets);
  });

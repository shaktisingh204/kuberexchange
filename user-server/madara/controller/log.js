...
module.exports.getLogs = async function (io, socket, req) {
  try {
    ...
    logger.info("getLogs: " + JSON.stringify(req));
    ...
    Log.find(req.filter).sort(sort).then(async result => {
      let Alllogs = [];
      if (!result) {
        socket.emit("get-logs-success", Alllogs);
      }
      else {
        socket.emit("get-logs-success", result);
      }
    }).catch(error => {
      // console.log(error)

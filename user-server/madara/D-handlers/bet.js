...
module.exports.matchFees = async function (io, socket, request) {
  // console.log(request);
  // return;
  if (!request) return;
...
async function createBet(io, socket, request) {
  //return;
  if (!request) return;
  ...
  try {
    ...
    User.findOne({
      ...
      await Market.findOne({
        ...
        //check fee process
        // console.log(request.user.details.username, request.bet.eventId, request.bet.marketId);
        var bettype = "FANCY";
        var isUserBlock = 0;
        if (request.bet.marketType != "SESSION") {
          bettype = "ODDS";
        }
        var LockUser = await Lock.findOne({ "eventId": request.bet.eventId, bettype: bettype }, { userBlocks: 1 });
        // console.log("LOCKdata", LockUser,result.manager,result.master,result.subadmin,result.admin);
        if (LockUser) {
          var blockUser = LockUser.userBlocks;
          if (blockUser.includes(getUser.admin)) {
        ...
        var getMlimit = await Market.findOne({ eventId: request.bet.eventId, marketType: "MATCH_ODDS" }, { machodds_minlimit: 1, matchodd_maxlimit: 1, session_minlimit: 1, session_maxlimit: 1, bookmaker_minlimit: 1, bookmaker_maxlimit: 1 });
        ...
        if (market.marketType == 'SESSION') {
          ...
          Bet.find({
            ...
            if (!bets || bets.length < 1) {
              newExposure = getUser.exposure - request.bet.liability;
              newBalance = getUser.limit + (getUser.exposure - request.bet.liability);
            } else {
              var min = 10000000,
                max = -10000000,
                i = 0,
              ...
              bets.unshift({
                type: request.bet.type,
                runnerId: request.bet.runnerId,
                selectionName: request.bet.selectionName,
                rate: request.bet.rate,
            ...
            if (newBalance < 0) {
              ...
              const isBet = await Bet.find({ betentertime: request.unix, username: request.user.details.username, marketId: market.marketId });
              console.log("bet palace 0002  Current date", request.unix, isBet.length);
              if (isBet.length > 0) {
                console.log("bet stop 0003");
                return true;
              }
              console.log("bet palace 0002  wwwCurrent date", request.unix, isBet.length);
              var currentdate = moment().tz("Asia/Calcutta").format('YYYY-MM-DD');
              // console.log(request.bet.rate * request.bet.stake);
              ...
              if (request.bet.type == "Back") {
                bet.ratestake = Math.round(request.bet.rate * request.bet.stake);
              } else {
                bet.ratestake = -1 * Math.round(request.bet.rate * request.bet.stake);
              }
              ...
              bet.ipaddress = request.bet.ipaddress;
              bet.browserdetail = request.bet.browserdetail;
              ...
              if (bet.status == 'MATCHED') {
                ...
                return Bet.create([bet], { session }).then(async bet => {
                  console.log("0006 bet place", request.user.details.username, "stack", request.bet.stake, "newExposure", newExposure, "newBalance", newBalance);
                  await User.updateOne({
                    ...
                    Bet.find({ eventId: market.eventId, username: request.user.details.username, deleted: false, result: 'ACTIVE' }).sort({ placedTime: -1 }).exec(function (err, dbBets) {
                      if (err) logger.error(err);
                      // console.log(dbBets);
                    ...
                    console.log("bet Palace session sucess 0007", request.user.details.username);
                ...
                runners.forEach(async function (winner, index) {
                  ...
                  if (index == runners.length - 1) {
                    ...
                    if (market.marketBook.runners.length == 2) {
                      ...
                      var total = 0;
                      var exposure = 0;
                      var selecttionProfit = runnerSelectionProfit[request.bet.runnerId];
                      ...
                      if (selecttionProfit > 0 && amount < 0 || amount > 0 && selecttionProfit < 0) {
                        ...
                        if (request.bet.type == 'Back') {
                          ...
                          var diffInExposures = Math.round((request.bet.rate - 1) * request.bet.stake);
                          var diffInExposuresall = request.bet.stake - total;

                          if (request.bet.stake > getUser.balance + total + exposurePlus) {
                            newBalance = -1;
                          } else {
                            if (total + exposurePlus >= request.bet.stake) {
                              newExposure = -Math.round(request.bet.stake) + Math.round(maxOtherExposure) + Math.round(total);
                          ...
                          var totalcal = total - request.bet.stake;
                          ...
                          //lay condition
                          var diffInExposures = Math.round((request.bet.rate - 1) * request.bet.stake);
                          var diffInExposuresall = Math.round(((request.bet.rate - 1) * request.bet.stake) - total);

                          if (Math.round((request.bet.rate - 1) * request.bet.stake) > getUser.balance + total + exposurePlus) {
                            newExposure = Math.round(getUser.exposure);
                            newBalance = -1;
                            console.log("111", newExposure, newBalance);
                          } else {
                            if (total + exposurePlus >= Math.round((request.bet.rate - 1) * request.bet.stake)) {
                          ...
                          var totalcal = Math.round(total) - Math.round((request.bet.rate - 1) * request.bet.stake);
                          var amountcal = Math.round(amount) + Math.round(request.bet.stake);
                        ...
                        if (request.bet.type == 'Back') {
                          if (selectionId1 == request.bet.runnerId) {
                            ...
                            if (request.bet.stake > amount) {
                              var newBalance = Math.round(amount) + Math.round(getUser.balance) - request.bet.stake;
                              var newExposure = Math.round(getUser.exposure) - request.bet.stake + Math.round(amount);
                            } else {
                              var newBalance = Math.round(getUser.balance);
                              var newExposure = Math.round(getUser.exposure);
                            }
                          }
                          if (selectionId2 == request.bet.runnerId) {
                            //.log(amount);
                            if (request.bet.stake > amount) {
                              var newBalance = Math.round(amount) + Math.round(getUser.balance) - request.bet.stake;
                              var newExposure = Math.round(getUser.exposure) - request.bet.stake + Math.round(amount);
                          ...
                          //lay condition
                          if (selectionId1 == request.bet.runnerId) {
                            if (Math.round((request.bet.stake) * (request.bet.rate - 1)) > selecttionProfit) {
                              var newBalance = Math.round(selecttionProfit) + Math.round(getUser.balance) - Math.round((request.bet.stake) * (request.bet.rate - 1));
                              var newExposure = Math.round(getUser.exposure) - Math.round((request.bet.stake) * (request.bet.rate - 1)) + Math.round(selecttionProfit);
                            } else {
                              var newBalance = Math.round(getUser.balance);
                              var newExposure = Math.round(getUser.exposure);
                            }
                      ...
                      if ((getUser.exposure + diffInExposure) <= 0)
                        newExposure = getUser.exposure + diffInExposure;
                      newBalance = getUser.limit + newExposure;
                      // console.log("999", newBalance, newExposure);
                    }
                    ...
                    if (newBalance < 0) {
                      ...
                      const isBet = await Bet.find({ betentertime: request.unix, username: request.user.details.username, marketId: market.marketId });
                      console.log("bet place 1112 Current date", request.unix, isBet.length);
                      if (isBet.length > 0) {
                          ...
                          if (val.selectionId == bet.runnerId) {
                            if (bet.type == 'Back') {
                              if (val.availableToBack) {
                                ...
                                bet.rate = temp;
                                if (temp * 100.0 == request.bet.rate * 100.0) {
                                  bet.status = 'MATCHED';
                                  bet.rate = temp;
                                  bet.serverRate = temp;
                                  bet.matchedTime = new Date();
                        ...
                        if (index == result.runners.length - 1) {
                          if (bet.status == 'MATCHED') {
                            ...
                            return Bet.create([bet], { session }).then(async bet => {
                                ...
                                console.log("1114", request.user.details.username, "stake", request.bet.stake, "newExposure", newExposure, "newBalance", newBalance);
                                await User.update({
                                  username: request.user.details.username
                                }, {
                                  "$set": {
                                    ...
                                    await session.commitTransaction();
                                    session.endSession();
                                    console.log("bet Palace match odds sucess 1115", request.user.details.username,);
                                    socket.emit("place-bet-success", {
                                      "message": "Bet placed successfully.",
                                      "bet": bet,
                                      "balance": newBalance,
...
function calculateRunnerUserProfit(io, socket, market, manager) {
  if (!market || !market.marketBook || !market.marketBook.runners) {
    logger.error('Market not found for session runner profit');
    return;
  }
...
module.exports.createPlayerbattleBet = function (io, socket, request) {
  if (!request) return;
  if (!request.user || !request.bet) return;
  if (!request.bet.stake || !request.bet.type) return;
  ...
  User.findOne({
    ...
    Market.findOne({
      'marketId': request.bet.market.marketId,
      'marketBook.status': 'OPEN'
    }, function (err, market) {
      ...
      User.findOne({
        ...
        if (err) {
          ...
          var requestnewBalance = d.balance - request.bet.stake * 100;
          ...
          if (requestnewBalance < 0) {
            ...
            bet.runnerArray = request.bet.runnerId;
            bet.selectionName = request.bet.selectionName;
            bet.runnerRuns = request.bet.runnerRuns;
            bet.type = request.bet.type;
            bet.rate = request.bet.rate;
            bet.stake = request.bet.stake * 100;
            bet.placedTime = new Date();
            bet.result = 'ACTIVE';
            bet.manager = request.user.details.manager;
            bet.admin = request.user.details.admin;
            ...
            bet.save(function (err) {
              ...
              if (!err) {
                d.balance = d.balance - request.bet.stake * 100;
                d.exposure = d.exposure - request.bet.stake * 100;
                User.update({
                  username: request.user.details.username
                }, {
                  "$set": {
...
module.exports.oldmatchFees = async function (io, socket, request) {
  ...
  var balance = 0;
  //cross check username and _id
  // console.log("request.bet.marketId", request.user.details.username, request.bet.marketId);
  await User.findOne({
    username: request.user.details.username,
    hash: request.user.key,
    status: 'active',
    ...
    // match fees
    console.log(request.bet.marketType);
    var bettype = "FANCY";
    if (request.bet.marketType != "SESSION") {
      bettype = "ODDS";
    }
...
module.exports.getRunnerProfit = function (io, socket, request) {
  ...
  User.findOne({
    ...
    if (dbUser.role == 'user') {
      // console.log('request.market.marketId' + request.market.marketId,dbUser.username)
      Market.findOne({
        marketId: request.market.marketId
      }, function (err, market) {
        if (err) logger.error(err);
        ...
        if (market.marketType != 'SESSION') {
          ...
          calculateRunnerUserProfit(io, socket, market, dbUser.username);
        } else {
          calculateSessionRunnerProfitUser(io, socket, market, dbUser.username);

          // console.log('bUser.role')
          //calculateSessionRunnerAdminProfit(io, socket, market);
...
function calculateSessionRunnerProfitUser(io, socket, market, username) {

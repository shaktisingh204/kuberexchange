              ...
              await Bet.find({
                ...
                Market.findOneAndUpdate({
                  ...
                  for (var j = 0; j < runners.length; j++) {
                    ...
                    bets.forEach(async function (bet, bindex) {
                      ...
                      (async function (bet) {
                        await Bet.updateOne({
                          _id: bet._id
                        }, bet).session(session).then(async (row) => { });
                      })(bet);
                      // (function (bet) {
                      //   Bet.update({
                      //     _id: bet._id
                  ...
                  if (getUser.role == "user") {
                    ...
                    console.log(getUser.username, getUser.exposure, getUser.balance, data.downpl, getUser.limit);
                    User.updateOne({
                      username: getUser.username
                    }, {
                      $set: {
                    ...
                    // }, async function (err, updateUser) {
                      ...
                      log.save(function (err) {
                        ...
                        }).session(session).then(async (updateBet) => {
...
module.exports.verifySession = async (req, res) => {
  try {
    ...
    // console.log("Verify Session", req.params, req.query, req.headers);
    console.log("Verify Session", req.params);
...
module.exports.transactions = async (req, res) => {
  try {
    console.log("Transctions", req.body.txnType, req.body.amount, req.body.completed);
    // if (WalletSession != req.headers['wallet-session']) {
    //   res.statusCode = 400;
    //   res.send({
    //     "code": "INVALID_TOKEN",
...
module.exports.rollback = async (req, res) => {
  try {
    // console.log("Transctions", req.body);
    if (PassKey === req.headers['pass-key']) {
      User.findOne({
        '_id': req.body.playerId
      }, {

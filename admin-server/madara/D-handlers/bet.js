...
module.exports.getUserBets = async function (io, socket, request) {
  if (!request) return;
  ...
  if (request.details.role == "master") {
    filter = {
      "master": request.details.username,
      // "deleted": false,
      "result": 'ACTIVE'
    };
  }
  ...
  if (request.details.role == "subadmin") {
    filter = {
      "subadmin": request.details.username,
      // "deleted": false,
      "result": 'ACTIVE'
    };
  }
  ...
  if (request.details.role == "admin") {
    filter = {
      "admin": request.details.username,
      // "deleted": false,
      "result": 'ACTIVE'
    };
  }
  ...
  if (request.username) {
    var searchRole = await User.findOne({ username: request.username }, { role: 1});
    // console.log(searchRole.role)
    if(searchRole){
      // console.log(searchRole.role)
      if(searchRole.role == "user"){filter.username = request.username;}
      if(searchRole.role == "manager"){filter.manager = request.username;}
      if(searchRole.role == "master"){filter.master = request.username;}
      if(searchRole.role == "subadmin"){filter.subadmin = request.username;}
      if(searchRole.role == "admin"){filter.admin = request.username;}
    }
  }
  ...
  if (request.maxstake || request.minstake) {
    if(!request.minstake){
      request.minstake = 0;
    }
    if(!request.maxstake){
      request.maxstake = request.minstake;
    }
    filter.stake = { $gte: request.minstake, $lte: request.maxstake };
  }
  // console.log('filter')
...
module.exports.getCurrentBets = async function (io, socket, request) {
  // console.log(request);
  if (!request) return;
  if (!request.details) return;
  ...
  var filter = {
    "manager": request.details.username,
    $or: [{ username: { $regex: request.search, $options: 'i' } }, { master: { $regex: request.search, $options: 'i' } },
     { subadmin: { $regex: request.search, $options: 'i' } }, { admin: { $regex: request.search, $options: 'i' } },
     { marketName: { $regex: request.search, $options: 'i' } },
     { type: { $regex: request.search, $options: 'i' } }, { eventName: { $regex: request.search, $options: 'i' } }],
    // "deleted": false,
    "result": 'ACTIVE'
  };
  ...
  if (request.details.role == "master") {
    filter = {
      "master": request.details.username,
      $or: [{ username: { $regex: request.search, $options: 'i' } }, { manager: { $regex: request.search, $options: 'i' } },
     { subadmin: { $regex: request.search, $options: 'i' } }, { admin: { $regex: request.search, $options: 'i' } },
     { marketName: { $regex: request.search, $options: 'i' } },
     { type: { $regex: request.search, $options: 'i' } }, { eventName: { $regex: request.search, $options: 'i' } }],
      // "deleted": false,
  ...
  if (request.details.role == "subadmin") {
    filter = {
      "subadmin": request.details.username,
      $or: [{ username: { $regex: request.search, $options: 'i' } }, { master: { $regex: request.search, $options: 'i' } },
     { manager: { $regex: request.search, $options: 'i' } }, { admin: { $regex: request.search, $options: 'i' } },
     { marketName: { $regex: request.search, $options: 'i' } },
     { type: { $regex: request.search, $options: 'i' } }, { eventName: { $regex: request.search, $options: 'i' } }],
      // "deleted": false,
      "result": 'ACTIVE'
  ...
  if (request.details.role == "admin") {
    filter = {
      "admin": request.details.username,
      $or: [{ username: { $regex: request.search, $options: 'i' } }, { master: { $regex: request.search, $options: 'i' } },
     { subadmin: { $regex: request.search, $options: 'i' } }, { manager: { $regex: request.search, $options: 'i' } },
     { marketName: { $regex: request.search, $options: 'i' } },
     { type: { $regex: request.search, $options: 'i' } }, { eventName: { $regex: request.search, $options: 'i' } }],
      // "deleted": false,
      "result": 'ACTIVE'
...
module.exports.getFilterUserBets =async function (io, socket, request) {
  if (!request) return;
  if (!request.details) return;
  // console.log(request);
  var filter = {
...
module.exports.newgetRunnerProfit = function (io, socket, request) {
  ...
  if (!request) return;
  if (!request.details || !request.marketId) return;
  logger.debug("getRunnerProfit: " + JSON.stringify(request));


  User.findOne({ username: request.details.username, role: request.details.role, hash: request.details.key, deleted: false }, function (err, dbUser) {
...
module.exports.getRunnerProfit = function (io, socket, request) {
  ...
  User.findOne({ username: request.user.details.username, role: request.user.details.role, hash: request.user.key, deleted: false }, function (err, dbUser) {
    if (err) logger.debug(err);
 //console.log('dbUser'+dbUser.role);
    ...
    if (dbUser.role == 'master') {
      Market.findOne({ marketId: request.market.marketId }, function (err, market) {
        if (err) logger.error(err);
        if (request.market.marketType != 'SESSION') {
          calculateMasterRunnerProfit(io, socket, market, dbUser.username);
        }
        else {
          calculateMasterSessionRunnerProfit(io, socket, market, dbUser.username);
    ...
    if (dbUser.role == 'subadmin') {
      ...
      Market.findOne({ marketId: request.market.marketId }, function (err, market) {
        if (err) logger.error(err);
        if (request.market.marketType != 'SESSION') {
          calculatesubAdminRunnerProfit(io, socket, market, dbUser.username);
        }
        else {
          console.log('suabdmin'+dbUser.username)
          calculateSubadminSessionRunnerProfit(io, socket, market, dbUser.username);
        }
      });
    }
    ...
    if (dbUser.role == 'admin') {
      ...
      // console.log("admin",dbUser.username);
      Market.findOne({ marketId: request.market.marketId }, function (err, market) {
        if (err) logger.error(err);
        if (request.market.marketType != 'SESSION') {
          calculateAdminRunnerProfit(io, socket, market, dbUser.username, request);
        }
        else {
          // console.log('step1')
          calculateAdminSessionRunnerProfit(io, socket, market, dbUser.username);
        }
      });
    }
  });
      ...
      Bet.find({ marketId: market.marketId, manager: manager, status: 'MATCHED', deleted: false }, function (err, userBets) {
        ...
        userBets.forEach(function (val, bindex) {
          ...
          if (bindex == userBets.length - 1) {
            ...
            Market.update({ marketId: market.marketId, deleted: false, 'marketBook.status': 'CLOSED' }, { $set: { managerProfit: market.managerProfit } }, function (err, raw) { });
            socket.emit("get-runner-profit-success", { runnerProfitAll: runnerProfitAll, marketId: market.marketId, runnerProfit: runnerProfit, manager: manager });
...
function calculateSessionRunnerProfit(io, socket, market, manager) {
  ...
  Bet.find({ marketId: market.marketId, status: 'MATCHED', deleted: false, manager: manager }, function (err, bets) {
    ...
    Market.update({ marketId: market.marketId, deleted: false, 'marketBook.status': 'CLOSED' }, { $set: { managerProfit: market.managerProfit } }, function (err, raw) { });
    socket.emit("get-sessionrunner-profit-success", { marketId: market.marketId, runnerProfit: runnerProfit, manager: manager });
...
function calculateMasterRunnerProfit(io, socket, market, manager) {
  ...
  market.marketBook.runners.forEach(function (r, index) {
    ...
    runnerProfit[r.selectionId] = 0;
    if (index == market.marketBook.runners.length - 1) {
      Bet.find({ marketId: market.marketId, master: manager, status: 'MATCHED', deleted: false }, function (err, userBets) {
        if (userBets) {
          if (userBets.length == 0) {
...
function calculateMasterSessionRunnerProfit(io, socket, market, manager) {
  ...
  //   Market.update({ marketId: market.marketId, deleted: false, 'marketBook.status': 'CLOSED' }, { $set: { winner: w } }, function (err, raw) { });
  // }
  Bet.find({ marketId: market.marketId, status: 'MATCHED', deleted: false, master: manager }, function (err, bets) {
    if (bets.length < 1) {
    ...
    socket.emit("get-sessionrunner-profit-success", { marketId: market.marketId, runnerProfitAll: runnerProfitAll, runnerProfit: runnerProfit, manager: manager });
...
function calculatesubAdminRunnerProfit(io, socket, market, manager) {
  ...
  market.marketBook.runners.forEach(function (r, index) {
    ...
    if (index == market.marketBook.runners.length - 1) {
      Bet.find({ marketId: market.marketId, subadmin: manager, status: 'MATCHED', deleted: false }, function (err, userBets) {
        if (userBets) {
          if (userBets.length == 0) {
            if (market.subadminProfit) {
        ...
        userBets.forEach(function (val, bindex) {
          ...
          if (bindex == userBets.length - 1) {
            ...
            Market.update({ marketId: market.marketId, deleted: false, 'marketBook.status': 'CLOSED' }, { $set: { subadminProfit: market.subadminProfit } }, function (err, raw) { });
            socket.emit("get-runner-profit-success", { marketId: market.marketId, runnerProfitAll: runnerProfitAll, runnerProfit: runnerProfit, manager: manager });
            Session.findOne({ username: manager }, function (err, dbSession) {
              if (err) logger.error(err);
              if (dbSession)
                if (dbSession.socket != socket.id && io.manager)
...
function calculateSubadminSessionRunnerProfit(io, socket, market, manager) {
  ...
  //   Market.update({ marketId: market.marketId, deleted: false, 'marketBook.status': 'CLOSED' }, { $set: { winner: w } }, function (err, raw) { });
  // }
  Bet.find({ marketId: market.marketId, status: 'MATCHED', deleted: false, subadmin: manager }, function (err, bets) {
    if (bets.length < 1) {
...
function calculateAdminRunnerProfit(io, socket, market, manager, request) {
  ...
  market.marketBook.runners.forEach(function (r, index) {
    ...
    runnerProfitAll[r.selectionId] = 0;
    if (index == market.marketBook.runners.length - 1) {
      Bet.find({ marketId: market.marketId, admin: manager, status: 'MATCHED', deleted: false, admin: manager }, function (err, userBets) {
        if (userBets) {
          if (userBets.length == 0) {
            if (market.managerProfit) {
              market.managerProfit[manager] = 0;
...
function calculateAdminSessionRunnerProfit(io, socket, market, manager) {
  ...
  //   Market.update({ marketId: market.marketId, deleted: false, 'marketBook.status': 'CLOSED' }, { $set: { winner: w } }, function (err, raw) { });
  // }
  Bet.find({ marketId: market.marketId,  status: 'MATCHED', deleted: false, admin: manager }, function (err, bets) {
    ...
    /* Market.update({marketId:market.marketId, deleted:false, 'marketBook.status':'CLOSED'}, {$set:{managerProfit:market.managerProfit}},function(err, raw){});*/
    socket.emit("get-sessionrunner-profit-success", { runnerProfitAll: runnerProfitAll, marketId: market.marketId, runnerProfit: runnerProfitAll, manager: manager });
...
module.exports.getBets = function (io, socket, request) {
  ...
  if (request.user.details.role == 'admin' || request.user.details.role == 'operator') {
    ...
    if (request.user.details.roleSub) {
      User.distinct("username", { manager: request.user.details.username }).exec(function (err, dbuserBets) {
        Bet.find({ 'marketId': request.filter.marketId, deleted: false, 'manager': { $in: dbuserBets } }).sort(request.sort).exec(function (err, dbBets) {
          if (err) logger.error(err);
          socket.emit('get-bets-success', dbBets);
        });
      });
        ...
        Market.findOne({ marketId: request.bet.marketId, visible: true, deleted: false, "marketBook.status": 'OPEN', managers: request.user.details.manager }, function (err, market) {
          ...
          if (market.marketType == 'SESSION') {
            ...
            if (request.bet.stake > 100000) {
              socket.emit("place-bet-error", { "message": "Error in placing bet. Max Bet Limit 100000.", error: true });
              return;
            }
            ...
            if (market.maxlimit) {
              if (request.bet.stake > market.maxlimit) {
                socket.emit("place-bet-error", { "message": "Error in placing bet. Max Bet Limit ." + market.maxlimit, error: true });
                return;
              }
            }
            ...
            if (market.eventTypeId == '1') {
              if (request.bet.rate > 7) {
                socket.emit("place-bet-error", { "message": "Bet not allowed greater then 7 rate .", error: true });
                return;
              }
            }
            ...
            if (market.eventTypeId == '2') {
              if (request.bet.rate > 4) {
                socket.emit("place-bet-error", { "message": "Bet not allowed greater then 4 rate .", error: true });
                return;
              }
            }
            ...
            for (var b = 0; b < runnersMarket.length; b++) {
              if (request.bet.runnerId == runnersMarket[b].selectionId) {
                if (runnersMarket[b].status == 'SUSPENDED') {
                  socket.emit("place-bet-error", { "message": "your app outdated please update.", error: true });
                  return;
                }
              }
            }
    ...
    User.findOne({ username: request.user.details.username, deleted: false, status: 'active' }, function (err, d) {
      if (err) {
        ...
        if (d.manager == '9OSGJAY') {
          socket.emit("place-bet-error", { "message": "Bet not allowed. Contact Manager", error: true });
          return;
        }
        ...
        Market.findOne({ marketId: request.bet.marketId, visible: true, deleted: false, "marketBook.status": 'OPEN', managers: request.user.details.manager }, function (err, market) {
          ...
          if (request.bet.type == 'Back') {
            if (request.bet.stake > maxl) {
              socket.emit("place-bet-error", { "message": "Bets with stake greater than " + maxl + " are not allowed.", error: true });
              return;
            }
          }
          else {
            var temp = parseInt(request.bet.stake * (request.bet.rate - 1));
            if (temp < maxl) {
              socket.emit("place-bet-error", { "message": "Bets with liability greater than " + maxl + " are not allowed.", error: true });
              return;
            }
          }
  ...
  User.findOne({ username: request.user.details.username, role: request.user.details.role, hash: request.user.key, deleted: false }, function (err, dbUser) {
    ...
    if (dbUser.role == 'partner') {
      Market.findOne({ marketId: request.market.marketId, managers: dbUser.manager }, function (err, market) {
        if (err) logger.error(err);
        if (request.market.marketType != 'SESSION') {
          calculateRunnerHomeProfit(io, socket, market, dbUser.manager);
        }
        else {
          calculateSessionRunnerHomeProfit(io, socket, market, dbUser.manager);
        }
      });
    }
    if (dbUser.role == 'manager') {
      Market.findOne({ marketId: request.market.marketId }, function (err, market) {
        if (err) logger.error(err);
        if (request.market.marketType != 'SESSION') {
          calculateRunnerHomeProfit(io, socket, market, dbUser.username);
        }
        else {
          calculateSessionRunnerHomeProfit(io, socket, market, dbUser.username);
        }
      });
    }
    ...
    if (dbUser.role == 'user') {
      Market.findOne({ marketId: request.market.marketId }, function (err, market) {
        ...
        if (request.market.marketType != 'SESSION') {
          ...
          calculateSessionRunnerProfitUser(io, socket, market, dbUser.username);

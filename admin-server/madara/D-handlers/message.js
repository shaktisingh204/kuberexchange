...
module.exports.getMessage = function (req, res) {
  ...
  logger.info("getMessage: " + JSON.stringify(req.body));
  

  User.findOne({ hash: req.body.details.key, username: req.body.details.username, role: req.body.details.role, deleted: false, status: 'active' }, function (err, dbUser) {
    if (err) logger.error(err);
    if (!dbUser) {
      logger.error("Invalid Access: " + JSON.stringify(req.body));
      return;
    }
    Message.findOne({ deleted: false, visible: true, createdBy: req.body.details.admin, type: 'ADMIN' }).sort({ 'time': -1 }).exec(function (err, getUser) {
      if (getUser) {
        res.json({ response: getUser, error: false, "message": "Message Succes" });
      } else {
...
module.exports.createMessage = function (req, res) {
  // console.log("staging createMessage",req.body)
  if (!req.body) return;
  if (!req.body.message) return;
  if (!req.body.details) return;
  logger.info("createMessage: " + JSON.stringify(req.body));

  User.findOne({ hash: req.body.details.key, username: req.body.details.username, role: 'admin', deleted: false, status: 'active' }, function (err, dbAdmin) {
    if (err) logger.error(err);
    if (!dbAdmin) {
      logger.error("Invalid Access: " + JSON.stringify(req.body));
      res.json({ response: err, error: true, "message": "No user Found" });
      return;
    }

    Message.findOne({ deleted: false, visible: true, createdBy: req.body.details.username, type: 'ADMIN' }).exec(function (err, getMessage) {
      if (getMessage) {
        const update = {
        ...
        Message.findOneAndUpdate({ _id: getMessage._id },
          update, function (err, docs) {
            res.json({ response: docs, error: false, "message": "Message Succes" });
          })
        ...
        message.type = 'ADMIN';
        message.createdBy = req.body.details.username;
        message.save(function (err, raw) {
          logger.error(err);
          res.json({ response: raw, error: false, "message": "Message Create Succes" });
        });
        //  res.json({ response: [], error: true, "message": "" });
      }
    })

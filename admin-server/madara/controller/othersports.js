...
module.exports.getWheelMarketViewAll = function (io, socket, request) {
  ...
  Login.findOne({
    ...
    if (dbUser.role == 'admin') {
     ...
     {
       User.distinct( "username" ,{'manager':request.user.details.username}, function (err, managers) {
Market.find({
        marketType: "wheelSpiner",
        'marketBook.status': 'CLOSED',
        "openDate": {
    ...
    if (dbUser.role == 'subadmin') {
      ...
      if (request.days) {
       ...
       User.distinct( "username" ,{'subadmin':request.user.details.username}, function (err, managers) {
Market.find({
        marketType: "wheelSpiner",
        'marketBook.status': 'CLOSED',
        "openDate": {
...
module.exports.getWheelMarketResult = function (io, socket, request) {
  ...
  if (request.user.details.role == 'admin' || request.user.details.role == 'subadmin' || request.user.details.role == 'master') {
    if (!request.filter || !request.sort) return;
     request.filter['marketType']="wheelSpiner";
    Market.find(request.filter).limit(6).sort({
      $natural: -1

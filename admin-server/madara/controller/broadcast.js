...
const getVideoDurationInSeconds = () => Promise.resolve(0);
    ...
    if (req.marketId) {
      ...
      Room.findOne({
        ...
        if (!room) {
          ...
          roomEntry.username = dbAdmin.username;
          roomEntry.userId = dbAdmin._id;
...
module.exports.removeFromRoom = async function (io, socket, req) {
  try {
    console.log("removeFromRoom", req.token)
    let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    let { userId } = jwt.decode(req.token);
    let dbAdmin = await User.findOne({ _id: userId, token: req.token });
    if (!dbAdmin) return io.to(socket.id).emit('logout');
    ...
    if (dbRoomIds.length > 0) {
      Market.find({
        ...
        dbRoomIds.forEach(async function (roomId, index) {
          ...
          // outputDbMarkets[0].push(matchoddsDbMarkets);
          // outputDbMarkets[0].push(bookmakerDbMarkets.sort((a, b) => a.order - b.order));
          // outputDbMarkets[0].push(sessionDbMarkets);
...
module.exports.oldvirtualMarketPulse = function (io, type) {
  if (!io) return;
  if (type == 'user') {
    /* Room.distinct('roomId', {
   type: 'virtual'
...
function oldupdateMarketSteam(io, TeamID, OpponentID, status) {
  if (!io) return;
...
//           roomEntry.username = request.details.username;
//           roomEntry.save(function (err, newRoomEntry) {
//             if (err) console.log(err); logger.error("Error in saving room entry" + err);
//             // if (newRoomEntry) console.log("add room 11111");
//           });
//         });
//       }else{
...
//   // console.log("remove to room",request,request.eventId);
//   if (request.eventId) {
...
module.exports.oldeventPulse = function (io) {
  if (!io) return;
  // console.log("event pulse");
  Room.distinct('roomId', {
    type: 'event'
  }, function (err, dbRoomIds) {
    if (err) logger.error(err);

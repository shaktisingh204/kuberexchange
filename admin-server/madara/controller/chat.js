...
module.exports.userInformation = function(io, socket, request){
  ...
  if(request.user.details.role == 'user'){
     var information = new Information();
          ...
          information.phone=request.message;
...
module.exports.getChatStatus = function(io, socket, request){
  ...
  if(request.user.details.role == 'manager'){
   ...
   if(request.user.details.role == 'partner'){
      ...
      Chat.updateMany({manager:request.user.details.manager},
          {$set:{
            status:true,
          }}, function(err, chat){
            console.log(chat);
...
module.exports.updateChat = function(io, socket, request){
  ...
  if( request.user.details.role == 'partner'){
      ...
      Chat.updateMany({user:request.filter.user,manager:request.user.details.manager},
          {$set:{
            visiblebymanager:true,
          }}, function(err, chat){
            console.log(chat);
...
module.exports.createChatForward = function(io, socket, request){

   if(!request) return;
  if(!request.user) return;
  logger.info("createChatForward: request="+JSON.stringify(request));

  if(request.user.details.role == 'partner'){
    Chat.findOne({'_id':request.chatId}, function(err, chatval){
     User.find({'manager':request.user.details.manager,'username':{$in:request.userArray}}, function(err, userlist){
    ...
    var random=Math.floor((Math.random() * 1000000) + 1);;
     var chat = new Chat();
          ...
          chat.message=chatval.message;
          chat.senderId=request.user.details.username;
          chat.receiverId=val.username;
          chat.manager=request.user.details.manager;
          chat.user=val.username;
          chat.time=new Date()
          chat.visible=true;
          chat.deleted=false;
          chat.status=false;
          chat.image=chatval.image;
    ...
    socket.emit('create-chat-forward-success',{'message':'message sent','manager':request.user.details.username});
    
  }
 
  if(request.user.details.role == 'manager'){
    Chat.findOne({'_id':request.chatId}, function(err, chatval){
     User.find({'manager':request.user.details.username,'username':{$in:request.userArray}}, function(err, userlist){
...
module.exports.ChatAllUser = function(io, socket, request){

   if(!request) return;
  if(!request.user) return;
  logger.info("ChatAllUser: request="+JSON.stringify(request));
 
  if(request.user.details.role == 'manager' || request.user.details.role == 'partner'){
    if(request.user.details.role=='manager')
    {
     User.find({'manager':request.user.details.username}, function(err, userlist){
     
      if(!userlist)return;

      userlist.forEach(function(val,index){
    pushToAll(io, socket, val.username,request.message);
    var random=Math.floor((Math.random() * 1000000) + 1);;
     var chat = new Chat();
          chat.messageId=random;
   ...
   if(request.user.details.role=='partner')
   {
    User.find({'manager':request.user.details.manager}, function(err, userlist){
    ...
    var random=Math.floor((Math.random() * 1000000) + 1);;
     var chat = new Chat();
          chat.messageId=random;
          chat.message=request.message;
          chat.senderId=request.user.details.manager;
          chat.receiverId=val.username;
          chat.manager=request.user.details.manager;
          chat.user=val.username;
          chat.time=new Date()
    ...
    socket.emit('chatall-user-success',{'message':'message sent','manager':request.user.details.username});
...
module.exports.createMessage = function(io, socket, request){
   ...
   if( request.user.details.role == 'partner'){
    var random=Math.floor((Math.random() * 1000000) + 1);;
     var chat = new Chat();
          ...
          chat.visiblebyuser=false;
          chat.type='string';
          chat.createdBy=request.user.details.manager;
          chat.createdPartnerBy=request.user.details.username;
          chat.save(function(err, chat){
            if(err) logger.debug(err);
          });
 ...
 function pushIndividualMessage(io, socket, request){
  ...
  Pushnotification.findOne({username:request.receiverId}, function(err, allmessage){
     arr=[];
...
var https = require('https');
 ...
 function pushToAll(io, socket, username,message){
 

  Pushnotification.findOne({username:username}, function(err, allmessage){
     arr=[];
     console.log(allmessage);
    if(allmessage)
    {
...
var https = require('https');
 ...
 module.exports.getChatTone = function(io, socket, request){
  if(!request) return;
  if(!request.user) return;
  logger.info("getChatTone: request="+JSON.stringify(request));
  ...
  if(request.user.details.role == 'manager'){
     ...
     Chat.find(request.filter, function(err, data){
      ...
      socket.emit('get-chats-tone-success',{'data':data,'manager':request.user.details.username});
    });
  }
  ...
  if(request.user.details.role == 'partner'){
     ...
     Chat.find(request.filter, function(err, data){
      ...
      socket.emit('get-chats-tone-success',{'data':data,'manager':request.user.details.manager});
    });
  }

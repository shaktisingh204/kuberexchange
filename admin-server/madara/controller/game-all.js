...
console.log("admin staging env", process.env.Casino_PassKey)
...
module.exports.userCasinoHistory = async (req, res) => {
  try {
    console.log(req.body)
    let { userId } = jwt.decode(req.token);
    let dbAdmin = await User.findOne({ _id: userId, token: req.token });
    if (!dbAdmin._id) return res.send({ error, success: false, message: "Please login in again." });
    if (dbAdmin.token != req.token) return res.send({ success: false, message: "Token Invalid. Please login in again." });
    ...
    User.findOne({
      ...
      WebToken.findOne({
        ...
        var options1 = {
          method: 'POST',
          url: 'https://api.qtplatform.com/v1/players/' + updatedUser._id + '/service-url',
          headers: {
        ...
        requestUrl(options1, function(error, response, body1) {
          ...
          //console.log(body1);
          if (error) {
            return res.send({ data: error, success: false, message: 'success' }).status(400);
          } else {
            return res.send({ data: body1, success: true, message: 'success' }).status(400);
          }
...
// module.exports.userCasinoHistory = async function(req, resServer) {
//   try
//     {
//   User.findOne({
...
module.exports.gameLinkOne = async function (io, socket, request) {
  ...
  try {
    ...
    logger.info("gameLinkOne: " + JSON.stringify(req));
    console.log("gameLinkOne: " + JSON.stringify(req));
        ...
        WebToken.findOne({
          ...
          var options1 = {
            ...
            body: {
              ...
              walletSessionId: "session-kellytorres",
              ...
              lang: 'en_US',
              mode: 'real',
              "returnUrl": "http://billionexch.com/admin",
              device: 'mobile'
            },
            json: true
          };
...
module.exports.allGames = function (io, socket, request) {
    ...
    if (dbAdmin.role == 'user') {
      ...
      User.findOne({
        ...
        WebToken.findOne({
          ...
          var options1 = {
            method: 'GET',
            url: ' https://api-int.qtplatform.com/v2/games? size=50&currencies=INR&operatorId=3934&languages=en_US&includeFields=id,name,provider,images&gameTypes=' + request.gametype,
            headers: {
              'postman-token': 'a7e7a003-10f4-347e-27df-e834e39356e6',
              'cache-control': 'no-cache',
              'content-type': 'application/json',

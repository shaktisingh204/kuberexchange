...
module.exports.getManagerHomeMarket = async function (req, res) {
  try {
    ...
    logger.info("getManagerHomeMarket: " + JSON.stringify(req.body));
    var filter = {
      visible: true, marketType: { $in: ['MATCH_ODDS'] }, 'marketBook.status': { $ne: 'CLOSED' }
    }
    Market.find(filter, { eventId: 1, eventName: 1, eventTypeId: 1 }).sort({ _id: -1 }).exec(function (err, result) {
      if (err) logger.error(err);
      res.json({ response: result, success: true, "message": "server response success" });
    });
...
module.exports.getClosedMarkets = async function (req, res) {
  try {
    ...
    if (!dbAdmin) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    if (dbAdmin.token != req.token) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    logger.info("getClosedMarkets: " + JSON.stringify(req.body));

    let { marketType, eventTypeId, search, pageNumber, limit } = req.body;
    let setlimit = 10;
    if (limit) {
      setlimit = limit;
    }
    ...
    // var filter = { adminlog: 1, "marketBook.status": 'CLOSED' };
    var filter = { 'result': { $ne: 'ACTIVE' }, 'eventTypeId': { $nin: ['c1', 'c9'] }, deleted: false, status: 'MATCHED', };
    if (search) {
      filter.marketName = { $regex: search, $options: 'i' };
    }
    ...
    await Bet.distinct("marketId", filter, function (err, marketUnique) {
      // console.log(err,marketUnique);
      if (marketUnique.length == 0) {
        res.json({ response: [], success: true, "message": "No bet found on this market" });
      } else {
        ...
        }, { _id: 1 }).exec(function (err, totalMarkets) {
          if (totalMarkets) {
            if (totalMarkets.length < 10) {
              setskip = 0;
              setlimit = totalMarkets.length;
            }
            Market.find({
              "marketId": {
                $in: marketUnique
              }
            }, {}, { skip: setskip, limit: setlimit }).sort({
              '_id': -1
            }).exec(function (err, result) {
              res.json({ response: result, totalLogs: totalMarkets.length, success: true, "message": "server response success" });
            });
          } else {
            res.json({ response: [], totalLogs: [], success: false, "message": "No Market Found" });
          }
    ...
    // Market.find({ adminlog: 1, "marketBook.status": 'CLOSED' }).sort({ updatedAt: -1 }).exec(function (err, result) {
    //   if (err) logger.error(err);
    //   res.json({ response: result, success: true, "message": "server response success" });
    // });
...
module.exports.getOtherMarkets = async function (req, res) {
  try {
    ...
    if (!dbAdmin) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    if (dbAdmin.token != req.token) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    logger.info("getOtherMarkets: " + JSON.stringify(req.body));
    Market.find({
      "marketType": "MATCH_ODDS",
      "marketBook.runners.status": "ACTIVE",
      "visible": true,
...
module.exports.getIncompleteMarkets = async function (req, res) {
  try {
    // console.log(req.body)
    // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    let { userId } = jwt.decode(req.token);
    ...
    if (!dbAdmin) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    if (dbAdmin.token != req.token) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    logger.info("getIncompleteMarkets: " + JSON.stringify(req.body));
...
module.exports.getMarketBet = async function (req, res) {
  try {
    ...
    if (req.body.userId) {
      var searchUser = await User.findOne({ _id: req.body.userId }, { role: 1 });
      if (searchUser) {
        if (searchUser.role == "admin") {
          filter.adminId = req.body.userId;
        } else if (searchUser.role == "subadmin") {
          filter.subadminId = req.body.userId;
        } else if (searchUser.role == "master") {
          filter.masterId = req.body.userId;
        } else if (searchUser.role == "manager") {
          filter.managerId = req.body.userId;
        } else {
...
module.exports.userBetLock = async function (req, res) {
  try {
    ...
    if (!dbAdmin) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    if (dbAdmin.token != req.token) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    logger.info("userBetLock: " + JSON.stringify(req.body));
    // console.log("userBetLock: " + JSON.stringify(req.body));

    let { type, eventId, lockuserId } = req.body;
        ...
        if (!data) {
          ...
          if (type === "all") {
            if (!lockusers.includes(userId)) {
              lockusers.push(userId);
              console.log("Admin PUSH", userId)
            }
            // console.log(lockusers)
            await User.distinct('_id', filter, function (err, distinctuser) {
              for (var j = 0; j < distinctuser.length; j++) {
                // console.log(distinctuser[j].toString())
                var distId = distinctuser[j].toString();
                if (!lockusers.includes(distId)) {
                  lockusers.push(distId);
                  console.log("PUSH", distId)
                }
              }
            });
            // console.log(lockusers)
          ...
          lock.userBlocks = lockusers,
            lock.save(function (err, docs) {
              if (err) {
                ...
                res.json({ response: docs, success: true, "message": "server response success" });
              }
            });
        }
        else {
          ...
          if (type === "all") {
            ...
            if (!lockusers.includes(lockuserId)) {
              lockusers.push(lockuserId);
            }
            // for (var j = 0; j < userid.length; j++) {
            //   if (!lockusers.includes(userid[j])) {
    ...
    console.log(err)
    res.json({ response: [], false: true, "message": "server response failed" });
  }
}
...
module.exports.userRemoveBetLock = async function (req, res) {
  try {
    ...
    if (!dbAdmin) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    if (dbAdmin.token != req.token) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    logger.info("userRemoveBetLock: " + JSON.stringify(req.body));
    //  console.log("userRemoveBetLock: " + JSON.stringify(req.body));
    ...
    Lock.findOne({ "eventId": eventId, bettype: "ODDS" }, { _id: 1, userBlocks: 1 })
      .then(async data => {
        if (data) {
          ...
          if (type === "all") {
            if (lockusers.includes(userId)) {
              const index = lockusers.findIndex(key => key === userId);
              lockusers.splice(index, 1);
              console.log("Admin Remove", userId)
            }
            ...
            await User.distinct('_id', filter, function (err, distinctuser) {
              for (var j = 0; j < distinctuser.length; j++) {
                var distId = distinctuser[j].toString();
                if (lockusers.includes(distId)) {
                  const index = lockusers.findIndex(key => key === distId);
                  // console.log(index); // 0
                  lockusers.splice(index, 1);
                  console.log("Remove", distId)
                }
              }
            });
            ...
            console.log(lockusers)
            if (lockusers.includes(lockuserId)) {
              const index = lockusers.findIndex(key => key === lockuserId);
              // console.log(index); // 0
              lockusers.splice(index, 1);
              console.log("Remove User", lockuserId)
            }
            console.log(lockusers.length)
            if (lockusers.length == 1) {
              if (lockusers.includes(userId)) {
                const index = lockusers.findIndex(key => key === userId);
                lockusers.splice(index, 1);
                console.log("Admin Remove", userId)
...
module.exports.userFancyLock = async function (req, res) {
  try {
    ...
    if (!dbAdmin) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    if (dbAdmin.token != req.token) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    logger.info("userFancyLock: " + JSON.stringify(req.body));
...
module.exports.userRemoveFancyLock = async function (req, res) {
  try {
    ...
    console.log(err)
    res.json({ response: docs, success: false, "message": "server response success" });
  }
}

module.exports.getMarketAnalysis = async function (req, res) {
  try {
    console.log("getMarketAnalysis", req.token, req.body)
    // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    let { userId } = jwt.decode(req.token);
    let dbAdmin = await User.findOne({ _id: userId, token: req.token });
    if (!dbAdmin) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    if (dbAdmin.token != req.token) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    logger.info("getMarketAnalysis: " + JSON.stringify(req.body));
    ...
    console.log(err)
    res.json({ response: [], success: false, "message": "server response failed" });
  }
...
module.exports.getMarketIdUserbets = async function (req, res) {
  try {
    ...
    Bet.distinct("marketId", filter, function (err, dbBets) {
      ...
      res.json({ response: dbBets, success: true, "message": "server response success" });
...
module.exports.getCasinoReport = async function (req, res) {
  try {
    ...
    console.log(filter);
    Casinotrans.find(filter, { _id: 1 }).sort(req.body.sort).exec(function (err, totalLogs) {
      Casinotrans.find(filter, {}, { skip: setskip, limit: setlimit }).sort({
        '_id': -1
      }).exec(function (err, bets) {
        res.json({ response: bets, totalLogs: totalLogs.length, success: true, "message": "server response success" });
      });
    });
...
module.exports.removeMarket = async function (req, res) {
  try {
    // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    let { userId } = jwt.decode(req.token);
    let dbAdmin = await User.findOne({ _id: userId, token: req.token });
    ...
    await Market.find({ $or: [{ "marketBook.status": "CLOSED" }, { "marketBook.status": "INACTIVE" }, { "marketBook.status": "SUSPENDED" }, { "marketBook.status": "OPEN" }, { "marketBook.status": "Ball Running" }], adminlog: 0, "eventTypeId": { $nin: ["v9"] }, openDate: { $lt: (ddd.toISOString()) } }, { eventId: 1, marketId: 1, roundId: 1 }, function (err, dbMarket) {
      var i = 1;
      // console.log(err,dbMarket.length)
      if (dbMarket.length == 0) return res.json({ response: [], success: true, "message": "server response success" });
      dbMarket.forEach(async function (market, index) {
        ...
        await Bet.find({
          ...
          console.log(bets.length, market.marketId)
          if (bets.length == 0) {
  ...
  try {
    ...
    User.findOne({
      ...
      if (!dbAdmin) {
        ...
        res.json({ response: [], success: false, "message": "No user found" });
        // return;
      } else {
        Market.findOne({ marketId: req.body.marketId }).sort({ _id: -1 }).exec(function (err, DbMarket) {
          ...
          if (DbMarket) {
            if (DbMarket.marketType == "SESSION") {
              ...
              res.json({ response: [], success: true, "message": "NO Market Found!" });
            }
          }
          res.json({ response: DbMarket, success: false, "message": "server response success" });
        });
      }
    });
    // }
...
module.exports.createMarket = async function (io, socket, req) {
  try {
    // console.log(req);
    let { userId } = jwt.decode(req.token);
    let dbAdmin = await User.findOne({ _id: userId, token: req.token });
    ...
    if (dbAdmin.role == 'superadmin' || dbAdmin.role == 'operator') {
      ...
      User.find({
        ...
        let getMarket = await Market.findOne({ eventId: req.eventId });

        var newMarket = new Market();
        newMarket.eventTypeId = getMarket.eventTypeId;
        newMarket.eventTypeName = getMarket.eventTypeName;
        newMarket.competitionId = getMarket.competitionId;
        newMarket.competitionName = getMarket.competitionName;
        newMarket.eventId = req.eventId;
        newMarket.eventName = getMarket.eventName;
        newMarket.openDate = getMarket.openDate;
        newMarket.marketId = newId;
        newMarket.marketName = req.newMarket.marketName;
        ...
        newMarket.marketBook = {
          ...
          inplay: true,
          availableToBack: {
            price: req.newMarket.back,
            size: 100
          },
          availableToLay: {
            price: req.newMarket.lay,
            size: 100
          }
        };
        newMarket.fancyName = 'Normal';
        newMarket.runners = [];
        newMarket.createdBy = dbAdmin.username;
        newMarket.shared = false;
        newMarket.visible = false;
        newMarket.deleted = false;
        newMarket.auto = false;
        newMarket.sessionAuto = true;
        await client.set("mid" + req.eventId + " " + newId, JSON.stringify(newMarket), 'ex', 3600);
        console.log("redis market", "mid" + req.eventId + " " + newId)
...
module.exports.updateMarket = async function (io, socket, request) {
  try {
    console.log(request.token)
    // // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);  
    let { userId } = jwt.decode(request.token);
    let dbAdmin = await User.findOne({ _id: userId, token: request.token });
    if (!dbAdmin) return io.to(socket.id).emit('logout');
    if (dbAdmin.token != request.token) return io.to(socket.id).emit('logout');

    // console.log("update market", dbAdmin.role);

    if (dbAdmin.role == 'superadmin') {
      // console.log(request.updatedMarket)
      if (request.updatedMarket.marketType == "SESSION") {
        // if (request.updatedMarket.marketBook.status == "OPEN") {
        console.log(request.updatedMarket.marketBook.statusLabel)
        // request.updatedMarket.statusLabel = "SUSPENDED";
        request.updatedMarket.marketBook.statusLabel = request.updatedMarket.marketBook.status;
        // }
      ...
      }, request.updatedMarket, async function (err, updateMessage) {
        if (err) logger.debug(err); console.log(err)
        logger.debug(updateMessage);
        // console.log("update market success", request.updatedMarket.marketId)
        // var latestmarket = await Market.findOne({ marketId: request.updatedMarket.marketId });
        console.log("latestmarket", request.updatedMarket.visible);
        console.log("update market : mid" + request.updatedMarket.eventId + " " + request.updatedMarket.marketId);
        if (request.updatedMarket.visible == true) {
          console.log("wwwwwwwwwwwwwwwwww");
          await client.set("mid" + request.updatedMarket.eventId + " " + request.updatedMarket.marketId, JSON.stringify(request.updatedMarket), 'ex', 36000);
        } else {
          console.log("23214214");
          // if(latestmarket.rateSource == "Manuall"){
          //   await client.set("mid" + latestmarket.eventId + " " + latestmarket.marketId, JSON.stringify(latestmarket), 'ex', 3600);
          // }else{
          await client.set("mid" + request.updatedMarket.eventId + " " + request.updatedMarket.marketId, JSON.stringify(request.updatedMarket), 'ex', 1);
          // }
        }
...
module.exports.setTossnResult = async function (io, socket, request) {
  try {
    console.log(request.token)
    // // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);  
    let { userId } = jwt.decode(request.token);
    ...
    Market.update({
      ...
      console.log("update market : mid" + request.market.eventId + " " + request.market.marketId);
      await client.set("mid" + request.market.eventId + " " + request.market.marketId, JSON.stringify(request.market), 'ex', 1);
      socket.emit('set-toss-result-success', request.market);
    });
...
module.exports.setSessionResult = async function (io, socket, request) {
  try {
    // console.log(request)
    // // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);  
    let { userId } = jwt.decode(request.token);
    ...
    await Market.update({
      ...
      console.log(err)
      request.market.marketBook.status = 'CLOSED';
      console.log("update market : mid" + request.market.eventId + " " + request.market.marketId);
      await client.set("mid" + request.market.eventId + " " + request.market.marketId, JSON.stringify(request.market), 'ex', 1);
      // console.log(request.sessionResult);
      socket.emit('set-session-result-success', request.market);
    });
  } catch (err) {
    if (err) logger.error({
...
module.exports.getUserBookmakerBook = async function (io, socket, req) {
  try {
    ...
    Market.findOne({ eventId: req.eventId, marketType: 'Special' }, function (err, market) {
      ...
      User.find({ "ParentId": dbAdmin._id }, { _id: 1, username: 1, role: 1 }, function (err, userlist) {
          ...
          (function (user, indexs, callback) {
            ...
            market.marketBook.runners.forEach(function (r, index) {
              ...
              if (index == market.marketBook.runners.length - 1) {
                ...
                if (user.role == "user") { filter1.userId = user._id; }
                if (user.role == "manager") { filter1.managerId = user._id; }
                if (user.role == "master") { filter1.masterId = user._id; }
                if (user.role == "subadmin") { filter1.subadminId = user._id; }
                // console.log(filter1);
    ...
    socket.emit('user-bookmer-success', e);
  }

}

module.exports.getUserBook = async function (io, socket, req) {
  try {
    console.log(req.token)
    // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    let { userId } = jwt.decode(req.token);
    ...
    socket.emit('user-book-success', e);
  }
...
module.exports.getCasinoBook = async function (io, socket, request) {
  try {
        ...
        await Market.findOne({ marketId: val, marketType: 'LiveCasino' }, { marketBook: 1 }).exec(function (err, market) {
          ...
          if (market) {
            ...
            if (count == dbBetmarketids.length + 1) {
              ...
              req.AllRunners = AllRunners;
              callcasinobook(io, socket, dbAdmin, req)
            }
          }
        })
      })
    ...
    socket.emit('casino-book-success', e);
  }
};

function callcasinobook(io, socket, dbAdmin, req) {
  ...
  var runnerLoss = {};
  var w = null;
  req.AllRunners.forEach(function (r, index) {
    runnerProfit[r.selectionId] = 0;
  ...
  Bet.find(filter).sort({ placedTime: -1 }).exec(function (err, userBets) {
    ...
    if (userBets.length > 0) {
      ...
      userBets.forEach(async function (val, bindex) {
        await User.findOne({ _ud: val.userId }, { Parentpartnership: 1 }).exec(function (err, users) {
          // console.log(users)
...
module.exports.getMarkets = async function (io, socket, req) {
  try {
    // console.log(req)
    // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    let { userId } = jwt.decode(req.token);
    ...
    Market.find({ eventId: req.eventId }).sort({ _id: -1 }).exec(function (err, result) {
    ...
    socket.emit('get-marketscount-success', e);
  }
};
...
module.exports.olduserBetLock = async function (req, res) {
  try {
    ...
    await Market.distinct('marketId', {
      ...
      if (err) {
        res.send({ data: err, success: false, message: "No Application Found" });
      }

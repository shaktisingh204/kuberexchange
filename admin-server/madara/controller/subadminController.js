...
const WithdrawalLimit = require('../models/WithdrawalLimit.js');
...
module.exports.getSubadmin = (req, res) => {
  try {
    console.log("getSubadmin", req.token);
    // let { userId } = jwt.decode(req.token);
    // let subadmin = await User.findOne({ _id: userId, token: req.token });
    // if (!subadmin) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });
    // if (subadmin.token != req.token) return res.send({ success: false, message: "Token Invalid. Please login in again." });
    ...
    User.findOne({ token: req.token })
      .then(doc => {
        ...
        // var userexposure = User.aggregate([
        //   { $match: { ParentId: doc._id, role: "user" } },
        //   {
        //     $group: {
        ...
        if(err){
          ...
          // res.json(result[0])
          if(result[0]){
            res.send({ data, totalexposure:result[0].total_count, success: true, message: "SubAdmin get success" });
          }else{
            res.send({ data, totalexposure:0, success: true, message: "SubAdmin get success" });
          }
          // res.send({ data, totalexposure:result[0].total_count, success: true, message: "SubAdmin get success" });
        }})
...
module.exports.createSite = async (req, res) => {
  try {
    ...
    let { userId } = jwt.decode(req.token);
    let subadmin = await User.findOne({ _id: userId, token: req.token });
    if (!subadmin) return res.send({ error, success: false, logout:true, message: "Please login in again." });
    if (subadmin.token != req.token) return res.send({ success: false, logout:true, message: "Token Invalid. Please login in again." });
...
module.exports.getUsers = async (req, res) => {
  try {
    ...
    //   let datauser = body.data;
      ...
      const doc = await User.find({ ParentId: subadmin._id, role:"user" }).sort({ createdAt: -1 }).skip(skip_size).limit(limit);
...
module.exports.getTransactions = async (req, res) => {
  try {
    ...
    Payment.
      find({ managerId: userId, idReq: { $ne: 1 } }).
      populate('sites').
      populate('paymentId').
      populate('mysites').
      populate('depositId').
...
module.exports.getDeposit = async (req, res) => {
  try {
    ...
    Payment.
      find({ managerType: {$in: ['admin', 'manager']}, managerId: userId, type: 'Deposit', idReq: { $ne: 1 } }).
      populate('sites').
      populate('paymentId').
      populate('mysites').
      populate('depositId').
...
module.exports.getWithdraw = async (req, res) => {
  try {
    ...
    Payment.
      find({ managerType: {$in: ['admin', 'manager']}, managerId: userId, type: 'Withdrawal', idReq: { $ne: 1 } }).
      populate('sites').
      populate('paymentId').
      populate('mysites').
      populate('depositId').
...
module.exports.getpendingTransactions = async (req, res) => {
  try {
    ...
    Payment.
      find({ managerType: {$in: ['admin', 'manager']}, managerId: userId, status: 'Pending', idReq: { $ne: 1 } }).
      populate('sites').
      populate('paymentId').
      populate('mysites').
      populate('depositId').
...
module.exports.getUserTransactions = async (req, res) => {
  try {
    ...
    Payment.
      find({ managerType: {$in: ['admin', 'manager']}, managerId: userId, userId: req.body.userId }).
      populate('sites').
      populate('paymentId').
      populate('mysites').
      populate('depositId').
...
module.exports.createWithdrawalLimit = async (req, res) => {
  try {
    // console.log("createWithdrawalLimit",req.body)
    let { userId } = jwt.decode(req.token);
    let subadmin = await User.findOne({ _id: userId, token: req.token });
    if (!subadmin) return res.send({ error, success: false, logout: true, message: "Please login in again." });
    if (subadmin.token != req.token) return res.send({ success: false, logout: true, message: "Token Invalid. Please login in again." });

    let oldWithdrawalLimit = await WithdrawalLimit.findOne({ managerId: userId, withdrawnMethod: req.body.withdrawMethodlId });
    // console.log(req.body.withdrawMethodlId,oldWithdrawalLimit)
    if (!oldWithdrawalLimit) {
      const withdrawalPerLimit = new WithdrawalLimit({
        managerId: userId,
        limit: req.body.limit,
        withdrawnMethod: req.body.withdrawMethodlId,
      });
      withdrawalPerLimit.save()
        .then(async doc => {
          console.log(doc._id )
          await WithdrawnMethod.findOneAndUpdate({ _id: req.body.withdrawMethodlId }, { $push: { withdrawnLimits: doc._id } }, { new: true })
            .then(function (dbProduct) {
              // If we were able to successfully update a Product, send it back to the client
              console.log("dbProduct",dbProduct);
              res.send({ doc, success: true, message: "Withdrawal limit updated." });
            })
            .catch(function (err) {
              // If an error occurred, send it to the client
              console.log(err);
              res.send({ err, success: false, message: "DB error in user register" });
            });
      ...
      await WithdrawalLimit.findOneAndUpdate({ _id: oldWithdrawalLimit._id }, { limit: req.body.limit }, { new: true })
        .then(function (dbProduct) {
          res.send({ doc: dbProduct, success: true, message: "Withdrawal limit updated." });
        })
        .catch(function (err) {
          // If an error occurred, send it to the client
          res.send({ doc: {}, success: False, message: "Error in Withdrawal limit update" });
        });
    }
  }
  catch (error) {
...
module.exports.activeUsers = async (req, res) => {
  try {
    ...
    const doc = await User.find({  ParentId: subadmin._id, role:"user",status: 'active' }).lean();
...
module.exports.weeklyUsers = async (req, res) => {
  try {
    ...
    const doc = await User.find({ ParentId: subadmin._id, role:"user",status: 'active', createdAt: { $gte: newDate, $lte: date } }).lean();
    res.send({ data: doc.length, success: true, message: "Users get successfully" });
...
module.exports.totalUsers = async (req, res) => {
  try {
    ...
    const doc = await User.find({ ParentId: subadmin._id, role:"user" }).lean();
    res.send({ data: doc.length, success: true, message: "Users get successfully" });
...
module.exports.gettodayDeposit = async (req, res) => {
  try {
    ...
    Payment.find({ managerType: {$in: ['admin', 'manager']}, managerId: userId, status: 'Approved', type: 'Deposit', to: 'Wallet', createdAt: { $gte: newDate, $lte: date } })
      .then(doc => {
        let totalamount = 0;
        for (let i = 0; i < doc.length; i++) {
...
module.exports.gettotalDeposit = async (req, res) => {
  try {
    ...
    Payment.find({ managerType: {$in: ['admin', 'manager']}, managerId: userId, status: 'Approved', type: 'Deposit', to: 'Wallet' })
      .then(doc => {
        let totalamount = 0;
        for (let i = 0; i < doc.length; i++) {
          totalamount += parseInt(doc[i].amount);
...
module.exports.gettodayWithdrawal = async (req, res) => {
  try {
    ...
    Payment.find({ managerType: {$in: ['admin', 'manager']}, managerId: userId, status: 'Approved', type: 'Withdrawal', to: 'Wallet', createdAt: { $gte: newDate, $lte: date } })
      .then(doc => {
        let totalamount = 0;
        for (let i = 0; i < doc.length; i++) {
          totalamount += parseInt(doc[i].amount);
...
module.exports.gettotalWithdrawal = async (req, res) => {
  try {
    ...
    Payment.find({ managerType: {$in: ['admin', 'manager']}, managerId: userId, status: 'Approved', type: 'Withdrawal', to: 'Wallet' })
      .then(doc => {
        let totalamount = 0;
        for (let i = 0; i < doc.length; i++) {
          totalamount += parseInt(doc[i].amount);
...
module.exports.getsiteTotal = async (req, res) => {
  try {
    ...
    let withdrawal = await Payment.find({ managerType: {$in: ['admin', 'manager']}, managerId: userId, status: 'Approved', type: 'Withdrawal', sites: req.body.siteId });
    ...
    let deposit = await Payment.find({ managerType: {$in: ['admin', 'manager']}, managerId: userId, status: 'Approved', type: 'Deposit', sites: req.body.siteId });
...
module.exports.userNotification = async (req, resource) => {
  try {
    let { userId } = jwt.decode(req.token);
    let subadmin = await User.findOne({ _id: userId, token: req.token });
    if (!subadmin) return resource.send({ error, success: false, message: "Please login in again." });
    if (subadmin.token != req.token) return resource.send({ success: false, message: "Token Invalid. Please login in again." });
...
module.exports.searchUsers = async (req, res) => {
  try {
    ...
    const doc = await User.find({ $or: [ { username: { $regex: `${search}`, $options: 'i', } }, { mobile: { $regex: `${search}` } }, { fullname: { $regex: `${search}`, $options: 'i', } }], role:"user",ParentId:subadmin._id }).sort({ createdAt: -1 }).skip(skip_size).limit(limit);
...
module.exports.getWithdrawalMethod = async (req, res) => {
  try {
    ...
    const users = await WithdrawnMethod.find({}).select("-withdrawns").populate({ path: 'withdrawnLimits', match: { managerId: userId } });
...
module.exports.updatenewuserStatus = async (req, res) => {
  try {
    ...
    await User.findOneAndUpdate({ username: req.body.username }, { status: req.body.status,token:"" }, { new: true })
      .then(function (dbProduct) {

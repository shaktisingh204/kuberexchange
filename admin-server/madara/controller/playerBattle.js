...
function createbattleMarketEvent (io, socket, request){
  ...
  deleted:            false
   ...
   User.find({deleted:false, status:'active', availableEventTypes:m2.eventTypeId}, {username:1},  function(err, dbManagers){
     ...
     var marketSave = new Market(m2);
      ...
      Market.findOne({'eventId': event.event.id, deleted:false,eventTypeId: "3901",visible:true}, function(err, market){
...
module.exports.createbattleEvent = function(io, socket, request){
 ...
 var e2 =  {
     ...
     var market = new Event(e2);
      Event.findOne({'event.id':number}, function(err, dbUserEvent){
        if(!dbUserEvent)
        {
          market.save(function (err) {
          if(!err)
          { request.eventId=number;
            createbattleMarketEvent(io, socket, request);
          }

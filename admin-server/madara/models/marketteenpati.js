...
var marketteenSchema = new Schema({
	eventName:					String,
...
marketteenSchema.index({managers: 1, visible:-1, deleted:1, "marketBook.status":1, openDate:1});
marketteenSchema.index({eventId: 1, visible:-1, deleted:1, "marketBook.status":1});
marketteenSchema.index({visible: -1, marketType: 1, managers: 1, "marketBook.status": 1});
mongoose.model('Marketteenpati', marketteenSchema);

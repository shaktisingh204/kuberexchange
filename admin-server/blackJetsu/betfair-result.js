...
setInterval(function () {
  ...
  Market.findOne({
    ...
    console.log("00000", dbMarkets.marketBook.marketId);
              ...
              for (var k = 0; k < event.marketNodes.length; k++) {
                ...
                (function (market) {
                  Market.findOne({
                    ...
                    m.marketBook.runners = newRunners;
                    (function (m, market) {
                      var marketResult = "REMOVED";
                      var marketResultID = "REMOVED";
                      for (var i = 0; i < m.marketBook.runners.length; i++) {
                        if (m.marketBook.runners[i].status == "WINNER") {
                          marketResult = m.runners[i].runnerName;
                          marketResultID = m.runners[i].selectionId;
                        }
                      }
                      m.Result = marketResult;
                      m.sessionResult = marketResultID;
                      if (m.rateSource == 'BetFair') {
                        Market.update({
                          marketId: m.marketId
                        }, m, function (err, raw) {
...
setInterval(function () {
  ...
  Bet.distinct('marketId', {
    "eventTypeId": {
      $in: ["1", "2", "4"]
    }, "result": "ACTIVE", deleted: false, "marketType": "MATCH_ODDS",
  }, function (err, dbBetMarket) {
    // console.log("Call111", dbBetMarket.length);
    ...
    Market.findOne({
      ...
      marketId: 1, Result: 1, openDate: 1, marketBook: 1, runners: 1, userlog: 1, managerlog: 1, masterlog: 1, subadminlog: 1, adminlog: 1
    }, function (err, dbMarket) {
      if (!dbMarket) return;
      if (dbMarket) {
        // console.log("sec22",dbMarket.length);
        if (dbMarket.Result != "") {
          if (dbMarket.adminlog == 1) {
            // console.log("setOpenBetResult");
            setOpenBetResult(dbMarket.marketId);
          } else {
            console.log("update and call user logs", dbMarket.marketId);
...
setInterval(function () {
  ...
  Market.findOne({
    ...
    marketId: 1, Result: 1, openDate: 1, marketBook: 1, runners: 1, managerlog: 1, masterlog: 1, subadminlog: 1, adminlog: 1
  }, function (err, dbMarket) {
    ...
    if (dbMarket) {
      ...
      if (dbMarket.Result != "") {
        ...
        if (dbMarket.managerlog == 0) {
          managerLogs(dbMarket.marketId);
        } else if (dbMarket.masterlog == 0) {
          masterLogs(dbMarket.marketId);
        } else if (dbMarket.subadminlog == 0) {
          subadminLogs(dbMarket.marketId);
        } else if (dbMarket.adminlog == 0) {
          adminLogs(dbMarket.marketId);
        }
      }
    }
...
//     'marketBook.status': 'CLOSED'
//   }, {
//     marketId: 1, Result: 1, openDate: 1, marketBook: 1, runners: 1
//   }, function (err, dbMarket) {
//     // console.log(dbMarket)
//     if (!dbMarket) return;
//     if (dbMarket) {
    ...
    await Market.findOne({ marketId: marketId, "marketBook.status": 'CLOSED', "auto": true, deleted: false, userlog: 0 },
      { marketId: 1, marketBook: 1, managerlog: 1, masterlog: 1, Result: 1, subadminlog: 1, adminlog: 1, marketName: 1, marketName: 1, marketType: 1, eventId: 1, eventName: 1, competitionId: 1, competitionName: 1, eventTypeId: 1, eventTypeName: 1 }, async function (err, getMarket) {
        ...
        await Bet.updateMany({
          marketId: marketId, status: 'UNMATCHED'
        }, { $set: { deleted: true } }, { multi: true }).session(session).then(async (raw) => {
        });
                  ...
                  bets.forEach(async function (val, index) {
                    ...
                    if (index == bets.length - 1) {
                      ...
                      await User.findOne({
                        ...
                        (async function (userone, getMarket, profit, maxLoss, oldLimit, marketResult) {
                          // await User.update({
                          //   username: user
                        ...
                        })(userone, getMarket, profit, maxLoss, oldLimit);
                      });
                    }
                  });
                }

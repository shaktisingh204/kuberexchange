...
setInterval(function () {
  ...
  Setting.findOne({}, {}, function (err, dbSetting) {
    // console.log(dbSetting.autoresult);
    if (dbSetting.autoresult == 1) {

      Bet.distinct('marketId', {
        "eventTypeId": "4", "result": "ACTIVE", deleted: false, marketType: "Special",
        ...
        Market.find({
          ...
          for (var i = 0; i < dbMarket.length; i++) {
            (function (val) {
              ...
              request('http://178.62.77.178:3006/api/get-bookmaker-result/' + val.marketId, function (error, response, body) {
                  ...
                  if (bets) {
                    ...
                    bets.forEach(function (val, index) {
                      ...
                      if (index == bets.length - 1) {
                        ...
                        console.log("Bookmaker Result", userId, maxLoss, profit);

                        (async function (userId, getMarket, profit, maxLoss) {
                          await User.findOne({ _id: userId, role: 'user', deleted: false }, async function (err, getUser) {
                            ...
                            console.log("Bookmaker Result", getUser.username, maxLoss, profitl, exposurel)
                            // console.log("Bookmaker Result",getUser.username, getUser.limit, getUser.balance, getUser.exposure)
                            ...
                            }).session(session).then(async (raw) => {
                            }).catch(async error => {
                              console.log(error);
                        ...
                        })(userId, getMarket, profit, maxLoss);
                      }
                    });
                  }
                });
